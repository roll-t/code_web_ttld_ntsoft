...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'TrinhDoHV', 'TrinhDoHVID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().TrinhDoHVID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().TrinhDoHVID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().TrinhDoHVID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaTrinhDoHV", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên trình độ học vấn", field: "TenTrinhDoHV", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "TrinhDoHVID", field: "TrinhDoHVID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/TrinhDoHV/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'TrinhDoHV', 'TrinhDoHVID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin trình độ học vấn');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#TrinhDoHVID').val(row.getData().TrinhDoHVID);
    SuaDuLieu(row.getData().TrinhDoHVID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin trình độ học vấn');
    const result = NTS.getAjax('/DanhMuc/TrinhDoHV/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#TrinhDoHVID').value(data.TrinhDoHVID);
        $('#MaTrinhDoHV').value(data.MaTrinhDoHV);
        $('#TenTrinhDoHV').value(data.TenTrinhDoHV);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraNhapSo($('#MaTrinhDoHV').value())) {
    ...
    saveData[0] = tempthem;
    saveData[1] = $('#TrinhDoHVID').value();
    saveData[2] = $('#MaTrinhDoHV').value();
    saveData[3] = $('#TenTrinhDoHV').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/TrinhDoHV/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#TrinhDoHVID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'TrinhDoHVID', ID: ID, TenBangHienTai: 'TrinhDoHV', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/TrinhDoHV/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
