...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'PhanLoaiLyDo', 'PhanLoaiLyDoID');
    //$('#reservationdate').inputmask('dd/mm/yyyy', { 'placeholder': 'dd/mm/yyyy' });
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().PhanLoaiLyDoID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().PhanLoaiLyDoID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().PhanLoaiLyDoID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaPhanLoaiLyDo", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên phân loại lý do", field: "TenPhanLoaiLyDo", formatter: 'textarea', hozAlign: "left", width: 400, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "PhanLoaiLyDoID", field: "PhanLoaiLyDoID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/PhanLoaiLyDo/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'PhanLoaiLyDo', 'PhanLoaiLyDoID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin phân loại lý do');
    $('#mdThemMoi').modal('show');
    $('#MaPhanLoaiLyDo').value('');
    $('#TenPhanLoaiLyDo').value('');
    $('#PhanLoaiLyDoID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#PhanLoaiLyDoID').val(row.getData().PhanLoaiLyDoID);
    SuaDuLieu(row.getData().PhanLoaiLyDoID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin phân loại lý do');
    const result = NTS.getAjax('/DanhMuc/PhanLoaiLyDo/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#PhanLoaiLyDoID').value(data.PhanLoaiLyDoID);
        $('#MaPhanLoaiLyDo').value(data.MaPhanLoaiLyDo);
        $('#TenPhanLoaiLyDo').value(data.TenPhanLoaiLyDo);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraKhongCoKyTuDacBiet($('#MaPhanLoaiLyDo').value())) {
        return NTS.canhbao("Mã không được chứa ký tự đăc biệt!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#PhanLoaiLyDoID').value();
    saveData[2] = $('#MaPhanLoaiLyDo').value();
    saveData[3] = $('#TenPhanLoaiLyDo').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/PhanLoaiLyDo/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#PhanLoaiLyDoID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    debugger;
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'PhanLoaiLyDoID', ID: ID, TenBangHienTai: 'PhanLoaiLyDo', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/PhanLoaiLyDo/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
