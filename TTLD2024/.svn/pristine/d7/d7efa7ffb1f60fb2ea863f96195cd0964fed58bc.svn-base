...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'ThoiGianSSLV', 'ThoiGianSSLVID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().ThoiGianSSLVID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().ThoiGianSSLVID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().ThoiGianSSLVID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaThoiGianSSLV", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên thời gian sẵn sàng làm việc", field: "TenThoiGianSSLV", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "ThoiGianSSLVID", field: "ThoiGianSSLVID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/ThoiGianSSLV/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'ThoiGianSSLV', 'ThoiGianSSLVID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới thời gian sẵn sàng làm việc');
    $('#mdThemMoi').modal('show');
    $('#MaThoiGianSSLV').value('');
    $('#TenThoiGianSSLV').value('');
    $('#ThoiGianSSLVID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#ThoiGianSSLVID').val(row.getData().ThoiGianSSLVID);
    SuaDuLieu(row.getData().ThoiGianSSLVID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thời gian sẵn sàng làm việc');
    const result = NTS.getAjax('/DanhMuc/ThoiGianSSLV/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#ThoiGianSSLVID').value(data.ThoiGianSSLVID);
        $('#MaThoiGianSSLV').value(data.MaThoiGianSSLV);
        $('#TenThoiGianSSLV').value(data.TenThoiGianSSLV);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraKhongCoKyTuDacBiet($('#MaThoiGianSSLV').value())) {
        return NTS.canhbao("Mã chỉ không được chứa ký tự đăc biệt!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#ThoiGianSSLVID').value();
    saveData[2] = $('#MaThoiGianSSLV').value();
    saveData[3] = $('#TenThoiGianSSLV').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/ThoiGianSSLV/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#ThoiGianSSLVID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'ThoiGianSSLVID', ID: ID, TenBangHienTai: 'ThoiGianSSLV', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/ThoiGianSSLV/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
