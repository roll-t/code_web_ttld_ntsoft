...
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Tên vai trò khi tham gia bãi công", field: "TenVaiTroTGLD", formatter: 'textarea', hozAlign: "left", width: 400, vertAlign: "middle", headerHozAlign: "center" },
...
function LoadDataTable() {
    ...
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/VaiTroTGBaiCong/GetAll", {});
    if (!GetAll.Err) {
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới vai trò khi tham gia bãi công');
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật vai trò khi tham gia bãi công');
    const result = NTS.getAjax('/DanhMuc/VaiTroTGBaiCong/LoadDuLieuSua', { id: ID });
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    var result = NTS.getAjax('/DanhMuc/VaiTroTGBaiCong/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
function XoaDuLieu(ID) {
    ...
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/VaiTroTGBaiCong/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
