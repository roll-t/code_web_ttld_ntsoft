...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'TrinhDoCMKT', 'TrinhDoCMKTID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().TrinhDoCMKTID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().TrinhDoCMKTID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().TrinhDoCMKTID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaTrinhDoCMKT", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên trình độ chuyên môn kỹ thuật", field: "TenTrinhDoCMKT", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "TrinhDoCMKTID", field: "TrinhDoCMKTID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/TrinhDoCMKT/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'TrinhDoCMKT', 'TrinhDoCMKTID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin trình độ chuyên môn kỹ thuật');
    $('#mdThemMoi').modal('show');
    $('#MaTrinhDoCMKT').value('');
    $('#TenTrinhDoCMKT').value('');
    $('#TrinhDoCMKTID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#TrinhDoCMKTID').val(row.getData().TrinhDoCMKTID);
    SuaDuLieu(row.getData().TrinhDoCMKTID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin trình độ chuyên môn kỹ thuật');
    const result = NTS.getAjax('/DanhMuc/TrinhDoCMKT/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#TrinhDoCMKTID').value(data.TrinhDoCMKTID);
        $('#MaTrinhDoCMKT').value(data.MaTrinhDoCMKT);
        $('#TenTrinhDoCMKT').value(data.TenTrinhDoCMKT);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraNhapSo($('#MaTrinhDoCMKT').value())) {
        return NTS.canhbao("Mã chỉ được chứa ký tự số!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#TrinhDoCMKTID').value();
    saveData[2] = $('#MaTrinhDoCMKT').value();
    saveData[3] = $('#TenTrinhDoCMKT').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/TrinhDoCMKT/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#TrinhDoCMKTID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'TrinhDoCMKTID', ID: ID, TenBangHienTai: 'TrinhDoCMKT', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/TrinhDoCMKT/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
