...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'LoaiHinhNoiLV', 'LoaiHinhNoiLVID');
});

function LoadCombo() {
    NTS.loadDataCombo({
        name: '#NhomLoaiHinhNoiLVID',
        ajaxUrl: '/DanhMuc/DungChung/GetNhomLoaiHinhNoiLV_Combo',
        columns: 2,
        indexValue: 0,
        indexText: 1,
        indexText1: 2,
...
var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().LoaiHinhNoiLVID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().LoaiHinhNoiLVID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().LoaiHinhNoiLVID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaLoaiHinhNoiLV", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên loại hình nơi làm việc", field: "TenLoaiHinhNoiLV", formatter: 'textarea', hozAlign: "left", width: 350, vertAlign: "middle", headerHozAlign: "center" },
        { title: "Thuộc nhóm loại hình nơi làm việc", field: "TenNhomLoaiHinhNoiLV", formatter: 'textarea', hozAlign: "left", width: 350, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "LoaiHinhNoiLVID", field: "LoaiHinhNoiLVID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/LoaiHinhNoiLV/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'LoaiHinhNoiLV', 'LoaiHinhNoiLVID');
...
$(document).on('click', '#btnThemMoi', function () {
    LoadCombo();
    $('#lblTieuDeThemMoi').text('Thêm mới loại hình nơi làm việc');
    $('#mdThemMoi').modal('show');
    $('#MaLoaiHinhNoiLV').value('');
    $('#TenLoaiHinhNoiLV').value('');
    $('#NhomLoaiHinhNoiLVID').value('');
    $('#LoaiHinhNoiLVID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#LoaiHinhNoiLVID').val(row.getData().LoaiHinhNoiLVID);
    SuaDuLieu(row.getData().LoaiHinhNoiLVID);
});
...
function SuaDuLieu(ID) {
    ...
    LoadCombo();
    $('#lblTieuDeThemMoi').text('Cập nhật loại hình nơi làm việc');
    const result = NTS.getAjax('/DanhMuc/LoaiHinhNoiLV/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#LoaiHinhNoiLVID').value(data.LoaiHinhNoiLVID);
        $('#MaLoaiHinhNoiLV').value(data.MaLoaiHinhNoiLV);
        $('#TenLoaiHinhNoiLV').value(data.TenLoaiHinhNoiLV);
        $('#NhomLoaiHinhNoiLVID').value(data.NhomLoaiHinhNoiLVID);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    //    return false;
    //}
    if (KiemTraKhongCoKyTuDacBiet($('#MaLoaiHinhNoiLV').value())) {
        return NTS.canhbao("Mã không được chứa ký tự đăc biệt!");
    }

    if (isAlphaNumericWithSpecialChars($('#TenLoaiHinhNoiLV').value())) {
        return NTS.canhbao("Tên loại hình nơi làm việc không chứa ký tự đặc biệt!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#LoaiHinhNoiLVID').value();
    saveData[2] = $('#MaLoaiHinhNoiLV').value();
    saveData[3] = $('#TenLoaiHinhNoiLV').value();
    saveData[4] = $('#NhomLoaiHinhNoiLVID').value();
    saveData[5] = $('#DienGiai').value();
    saveData[6] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/LoaiHinhNoiLV/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#LoaiHinhNoiLVID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'LoaiHinhNoiLVID', ID: ID, TenBangHienTai: 'LoaiHinhNoiLV', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/LoaiHinhNoiLV/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
