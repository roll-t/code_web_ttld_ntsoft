...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'DieuKienLaoDong', 'DieuKienLaoDongID');

});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().DieuKienLaoDongID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().DieuKienLaoDongID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().DieuKienLaoDongID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        //{ title: "Mã", field: "MaDieuKienLaoDong", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Mã", field: "MaDieuKienLaoDong", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên điều kiện lao động", field: "TenDieuKienLaoDong", formatter: 'textarea', hozAlign: "left", width: 400, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "DieuKienLaoDongID", field: "DieuKienLaoDongID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/DieuKienLaoDong/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'DieuKienLaoDong', 'DieuKienLaoDongID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#MaDieuKienLaoDong').prop('disabled', false);
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin điều kiện lao động');
    $('#mdThemMoi').modal('show');
    $('#MaDieuKienLaoDong').value('');
    $('#TenDieuKienLaoDong').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#DieuKienLaoDongID').val(row.getData().DieuKienLaoDongID);
    SuaDuLieu(row.getData().DieuKienLaoDongID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin điều kiện lao động');
    const result = NTS.getAjax('/DanhMuc/DieuKienLaoDong/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#DieuKienLaoDongID').value(data.DieuKienLaoDongID);
        $('#MaDieuKienLaoDong').value(data.MaDieuKienLaoDong);
        $('#TenDieuKienLaoDong').value(data.TenDieuKienLaoDong);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    saveData[0] = tempthem;
    saveData[1] = $('#DieuKienLaoDongID').value();
    saveData[2] = $('#MaDieuKienLaoDong').value();
    saveData[3] = $('#TenDieuKienLaoDong').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/DieuKienLaoDong/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#DieuKienLaoDongID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'DieuKienLaoDongID', ID: ID, TenBangHienTai: 'DieuKienLaoDong', CacBangKhongXet: [] });
    //console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/DieuKienLaoDong/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
