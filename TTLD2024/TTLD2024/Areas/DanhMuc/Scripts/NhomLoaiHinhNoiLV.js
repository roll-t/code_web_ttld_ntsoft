...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'NhomLoaiHinhNoiLV', 'NhomLoaiHinhNoiLVID');
});



var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().NhomLoaiHinhNoiLVID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().NhomLoaiHinhNoiLVID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().NhomLoaiHinhNoiLVID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaNhomLoaiHinhNoiLV", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên nhóm loại hình nơi làm việc", field: "TenNhomLoaiHinhNoiLV", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "NhomLoaiHinhNoiLVID", field: "NhomLoaiHinhNoiLVID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/NhomLoaiHinhNoiLV/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'NhomLoaiHinhNoiLV', 'NhomLoaiHinhNoiLVID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới nhóm loại hình nơi làm việc');
    $('#mdThemMoi').modal('show');
    $('#MaNhomLoaiHinhNoiLV').value('');
    $('#TenNhomLoaiHinhNoiLV').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#NhomLoaiHinhNoiLVID').val(row.getData().NhomLoaiHinhNoiLVID);
    SuaDuLieu(row.getData().NhomLoaiHinhNoiLVID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật nhóm loại hình nơi làm việc');
    const result = NTS.getAjax('/DanhMuc/NhomLoaiHinhNoiLV/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#NhomLoaiHinhNoiLVID').value(data.NhomLoaiHinhNoiLVID);
        $('#MaNhomLoaiHinhNoiLV').value(data.MaNhomLoaiHinhNoiLV);
        $('#TenNhomLoaiHinhNoiLV').value(data.TenNhomLoaiHinhNoiLV);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraKhongCoKyTuDacBiet($('#MaNhomLoaiHinhNoiLV').value())) {
        return NTS.canhbao("Mã không được chứa ký tự đăc biệt!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#NhomLoaiHinhNoiLVID').value();
    saveData[2] = $('#MaNhomLoaiHinhNoiLV').value();
    saveData[3] = $('#TenNhomLoaiHinhNoiLV').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/NhomLoaiHinhNoiLV/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#NhomLoaiHinhNoiLVID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'NhomLoaiHinhNoiLVID', ID: ID, TenBangHienTai: 'NhomLoaiHinhNoiLV', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/NhomLoaiHinhNoiLV/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
