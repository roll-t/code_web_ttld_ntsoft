    ...
    public class SlideController : Controller
    {
        ...
        public string GetAll()
        {
            try
            {
                ...
                DataTable duLieu = SqlHelper.ExecuteDataset(NTSSession.GetConnectionString2(), "Proc_GetAllSlide", null).Tables[0];
        ...
        public string LuuThongTin(object[] data)
        {
            try
            {
                ...
                SqlParameter[] para = {
                    ...
                    new SqlParameter("@SlideID",DungChung.NormalizationGuid(data[1].ToString())),
                    new SqlParameter("@TieuDe", data[2].ToString()),
                    new SqlParameter("@MoTa",data[3].ToString()),
                    new SqlParameter("@IMG", data[7].ToString()),
                    new SqlParameter("@DuongDan",data[4].ToString()),
                    new SqlParameter("@HienUuTien",data[5].ToString()),
                    new SqlParameter("@TrangThai",DungChung.NormalizationBoolean(data[6].ToString())),

                };
                DataSet duLieu = SqlHelper.ExecuteDataset(NTSSession.GetConnectionString1(), "Proc_LuuThongTin_Slide", para);
                if (Loai == "them")
                {
        ...
        public string LoadDuLieuSua(string id)
        {
            ...
            {
                SqlParameter[] para = {
                    new SqlParameter("@SlideID", DungChung.NormalizationGuid(id)),
                };
                DataTable duLieu = SqlHelper.ExecuteDataset(NTSSession.GetConnectionString1(), "Proc_GetSlideByID", para).Tables[0];
                ep.Result = duLieu;
                return JSonHelper.ToJson(ep);
            }
            catch (Exception ex)
        ...
        public string XoaDuLieu(string id)
        {
            try
            {
                ...
                DataSet duLieu = SqlHelper.ExecuteDataset(NTSSession.GetConnectionString1(), "Delete_Slide", para);
                if (duLieu.Tables[1].Rows[0][0].ToString() == "1")
                {
                    duLieu.Tables[0].TableName = "Slide";
