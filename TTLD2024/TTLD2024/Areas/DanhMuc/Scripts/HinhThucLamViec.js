...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'HinhThucLamViec', 'HinhThucLamViecID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().HinhThucLamViecID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().HinhThucLamViecID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().HinhThucLamViecID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaHinhThucLamViec", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên hình thức làm việc", field: "TenHinhThucLamViec", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "HinhThucLamViecID", field: "HinhThucLamViecID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/HinhThucLamViec/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'HinhThucLamViec', 'HinhThucLamViecID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin hình thức làm việc');
    $('#mdThemMoi').modal('show');
    $('#MaHinhThucLamViec').value('');
    $('#TenHinhThucLamViec').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#HinhThucLamViecID').val(row.getData().HinhThucLamViecID);
    SuaDuLieu(row.getData().HinhThucLamViecID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin hình thức làm việc');
    const result = NTS.getAjax('/DanhMuc/HinhThucLamViec/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#HinhThucLamViecID').value(data.HinhThucLamViecID);
        $('#MaHinhThucLamViec').value(data.MaHinhThucLamViec);
        $('#TenHinhThucLamViec').value(data.TenHinhThucLamViec);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraNhapSo($('#MaHinhThucLamViec').value())) {
        return NTS.canhbao("Mã chỉ được chứa ký tự số!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#HinhThucLamViecID').value();
    saveData[2] = $('#MaHinhThucLamViec').value();
    saveData[3] = $('#TenHinhThucLamViec').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/HinhThucLamViec/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#HinhThucLamViecID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'HinhThucLamViecID', ID: ID, TenBangHienTai: 'HinhThucLamViec', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/HinhThucLamViec/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
