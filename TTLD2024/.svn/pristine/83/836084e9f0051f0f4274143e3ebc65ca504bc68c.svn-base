...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'ChucVu', 'ChucVuID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().ChucVuID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().ChucVuID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().ChucVuID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaChucVu", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên chức vụ", field: "TenChucVu", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "ChucVuID", field: "ChucVuID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/ChucVu/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'ChucVu', 'ChucVuID');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#ChucVuID').val(row.getData().ChucVuID);
    SuaDuLieu(row.getData().ChucVuID);
});
...
function SuaDuLieu(ID) {
    ...
    const result = NTS.getAjax('/DanhMuc/ChucVu/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#ChucVuID').value(data.ChucVuID);
        $('#MaChucVu').value(data.MaChucVu);
        $('#TenChucVu').value(data.TenChucVu);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraNhapSo($('#MaChucVu').value())) {
        return NTS.canhbao("Mã chỉ được chứa ký tự số!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#ChucVuID').value();
    saveData[2] = $('#MaChucVu').value();
    saveData[3] = $('#TenChucVu').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/ChucVu/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'ChucVuID', ID: ID, TenBangHienTai: 'ChucVu', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/ChucVu/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
