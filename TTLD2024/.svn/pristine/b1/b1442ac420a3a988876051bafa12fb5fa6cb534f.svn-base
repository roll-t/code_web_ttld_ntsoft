...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'NganhSXKD', 'NganhSXKDID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().NganhSXKDID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().NganhSXKDID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().NganhSXKDID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaNganhSXKD", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên ngành sản xuất, kinh doanh", field: "TenNganhSXKD", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "NganhSXKDID", field: "NganhSXKDID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/NganhSXKD/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'NganhSXKD', 'NganhSXKDID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới ngành sản xuất, kinh doanh');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#NganhSXKDID').val(row.getData().NganhSXKDID);
    SuaDuLieu(row.getData().NganhSXKDID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật ngành sản xuất, kinh doanh');
    const result = NTS.getAjax('/DanhMuc/NganhSXKD/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#NganhSXKDID').value(data.NganhSXKDID);
        $('#MaNganhSXKD').value(data.MaNganhSXKD);
        $('#TenNganhSXKD').value(data.TenNganhSXKD);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraKhongCoKyTuDacBiet($('#MaNganhSXKD').value())) {
        return NTS.canhbao("Mã không chứa ký tự đăc biệt!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#NganhSXKDID').value();
    saveData[2] = $('#MaNganhSXKD').value();
    saveData[3] = $('#TenNganhSXKD').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/NganhSXKD/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#NganhSXKDID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'NganhSXKDID', ID: ID, TenBangHienTai: 'NganhSXKD', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/NganhSXKD/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
