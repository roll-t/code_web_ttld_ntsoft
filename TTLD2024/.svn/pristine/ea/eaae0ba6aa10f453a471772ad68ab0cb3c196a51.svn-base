...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'MucLuong', 'MucLuongID');

});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().MucLuongID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().MucLuongID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().MucLuongID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        //{ title: "Mã", field: "MaMucLuong", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Mã", field: "MaMucLuong", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên mức lương", field: "TenMucLuong", formatter: 'textarea', hozAlign: "left", width: 400, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "MucLuongID", field: "MucLuongID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/MucLuong/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'MucLuong', 'MucLuongID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#MaMucLuong').prop('disabled', false);
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin mức lương');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#MucLuongID').val(row.getData().MucLuongID);
    SuaDuLieu(row.getData().MucLuongID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin mức lương');
    const result = NTS.getAjax('/DanhMuc/MucLuong/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#MucLuongID').value(data.MucLuongID);
        $('#MaMucLuong').value(data.MaMucLuong);
        $('#TenMucLuong').value(data.TenMucLuong);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    saveData[0] = tempthem;
    saveData[1] = $('#MucLuongID').value();
    saveData[2] = $('#MaMucLuong').value();
    saveData[3] = $('#TenMucLuong').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/MucLuong/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'MucLuongID', ID: ID, TenBangHienTai: 'MucLuong', CacBangKhongXet: [] });
    //console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/MucLuong/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
