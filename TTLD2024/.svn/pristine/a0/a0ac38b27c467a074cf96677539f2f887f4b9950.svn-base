...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'YeuCauThem', 'YeuCauThemID');

});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().YeuCauThemID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().YeuCauThemID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().YeuCauThemID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        //{ title: "Mã", field: "MaYeuCauThem", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Mã", field: "MaYeuCauThem", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên yêu cầu thêm", field: "TenYeuCauThem", formatter: 'textarea', hozAlign: "left", width: 400, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "YeuCauThemID", field: "YeuCauThemID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/YeuCauThem/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'YeuCauThem', 'YeuCauThemID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#MaYeuCauThem').prop('disabled', false);
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin yêu cầu thêm');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#YeuCauThemID').val(row.getData().YeuCauThemID);
    SuaDuLieu(row.getData().YeuCauThemID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin yêu cầu thêm');
    const result = NTS.getAjax('/DanhMuc/YeuCauThem/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#YeuCauThemID').value(data.YeuCauThemID);
        $('#MaYeuCauThem').value(data.MaYeuCauThem);
        $('#TenYeuCauThem').value(data.TenYeuCauThem);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    saveData[0] = tempthem;
    saveData[1] = $('#YeuCauThemID').value();
    saveData[2] = $('#MaYeuCauThem').value();
    saveData[3] = $('#TenYeuCauThem').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/YeuCauThem/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#YeuCauThemID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'YeuCauThemID', ID: ID, TenBangHienTai: 'YeuCauThem', CacBangKhongXet: [] });
    //console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/YeuCauThem/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
