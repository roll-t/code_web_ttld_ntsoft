...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'QuocGia', 'QuocGiaID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().QuocGiaID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().QuocGiaID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().QuocGiaID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaQuocGia", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên quốc gia và vùng lãnh thổ", field: "TenQuocGia", formatter: 'textarea', hozAlign: "left", width: 350, vertAlign: "middle", headerHozAlign: "center" },
        { title: "Tên gọi chung", field: "TenGoiChung", formatter: 'textarea', hozAlign: "left", width: 350, vertAlign: "middle", headerHozAlign: "center" },
        { title: "Tên đầy đủ", field: "TenDayDu", formatter: 'textarea', hozAlign: "left", width: 350, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "QuocGiaID", field: "QuocGiaID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/QuocGia/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'QuocGia', 'QuocGiaID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới quốc gia và vùng lãnh thổ');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#QuocGiaID').val(row.getData().QuocGiaID);
    SuaDuLieu(row.getData().QuocGiaID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật quốc gia và vùng lãnh thổ');
    const result = NTS.getAjax('/DanhMuc/QuocGia/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#QuocGiaID').value(data.QuocGiaID);
        $('#MaQuocGia').value(data.MaQuocGia);
        $('#TenQuocGia').value(data.TenQuocGia);
        $('#TenGoiChung').value(data.TenGoiChung);
        $('#TenDayDu').value(data.TenDayDu);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraKhongCoKyTuDacBiet($('#MaQuocGia').value())) {
        return NTS.canhbao("Mã không chứa ký tự đăc biệt!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#QuocGiaID').value();
    saveData[2] = $('#MaQuocGia').value();
    saveData[3] = $('#TenQuocGia').value();
    saveData[4] = $('#TenGoiChung').value();
    saveData[5] = $('#TenDayDu').value();
    saveData[6] = $('#DienGiai').value();
    saveData[7] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/QuocGia/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'QuocGiaID', ID: ID, TenBangHienTai: 'QuocGia', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/QuocGia/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
