...
function XoaDuLieuTL(ID, ThamChieuID) {
    if (!QuyenXoa()) {
        return false;
    }
    ...
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                var result = NTS.getAjax('/QuanLy/DungChung/XoaDuLieuTaiLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTableTL(ThamChieuID);
...
$(document).on('change', '#fileVB_tailieu', function () {
    UploadTaiLieu_us('ThietLapWebSite'); //hàm dùng chung ở us TaiLieu
});
function UploadTaiLieu_us(pathChiTiet) {
    ...
    if (data.length > 0) {
        ...
        if (tempthemVB != "them") {
			let result = NTS.getAjax('/HeThong/ThietLapWebSite/LuuVanBan_DinhKem', { PathTemp: data, ID: $('#hdfTaiLieuLuuGiuID_us').value(), PathChiTiet: pathChiTiet, bangDk: 'CauHinhWebSite', cotDk: 'MaCauHinhID', cotDinhKem: 'Banner' });
            if (!result.Err) {
            ...
            let result = NTS.getAjax('/QuanLy/DungChung/LuuVanBan_DinhKem', { PathTemp: data, ID: $('#hdfTaiLieuLuuGiuID_us').value(), PathChiTiet: pathChiTiet, bangDk: 'TaiLieuLuuGiu', cotDk: 'TaiLieuLuuGiuID', cotDinhKem: 'DinhKem' });
            if (!result.Err) {
                let LuuFile = result.Result;
                let arrFile = LuuFile.split('*');
                for (let p = 0; p < arrFile.length - 1; p++) {
...
function LoadTinh() {
    ...
    NTS.loadDataCombo({
        name: '#TinhID',
        ajaxUrl: '/DanhMuc/DungChung/GetTinh',
        columns: 2,
        indexValue: 0,
        indexText1: 2,
        textShowTatCa: '-Chọn-',
...
function LoadDataTable() {
    table.clearData();
    const GetAll = NTS.getAjax("/HeThong/ThietLapWebSite/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD2('.checkDangSD', 'CauHinhWebSite', 'maCauHinhID');
$('#DangSD').on('change', function () {
    UpdateLabelDangSD(this);
});

var fmThaoTac = function (cell) {
	if (cell.getData().banner == "" || cell.getData().banner == null) {
		return `<div class="show-or-hide"><a class='text-primary btnSuaGrid1' title="Sửa" data='${cell.getData().maCauHinhID}'><i class="fa fa-pencil"></i></a></b>&ensp;<a class='text-danger btnXoaGrid1' title="Xoá" data='${cell.getData().maCauHinhID}'><i class='fa fa-trash-o'></i></a></div>`;
	} else {
		return `<div class="show-or-hide"><a class='text-primary btnSuaGrid1' title="Sửa" data='${cell.getData().maCauHinhID}'><i class="fa fa-pencil"></i></a></b>&ensp;<a class='text-danger btnXoaGrid1' title="Xoá" data='${cell.getData().maCauHinhID}'><i class='fa fa-trash-o'></i></a>&ensp;<a class='text-success btnXemAnh' title="Xem ảnh" data='${cell.getData().banner}'><i class='fa fa-picture-o'></i></a></div>`;
	}
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().maCauHinhID);
}
var table = new Tabulator("#Grid1", {
    ...
    columns: [
        { title: '<i class="fa fa-ellipsis-h"></i>', headerHozAlign: "center", hozAlign: "center", formatter: fmThaoTac, width: 80, headerSort: false, frozen: true, vertAlign: "middle", print: false },
        { title: "maCauHinhID", field: "maCauHinhID", visible: false, print: false },
		{ title: "banner", field: "banner", visible: false, print: false },
        { title: "Mã", field: "maCauHinh", formatter: 'textarea', headerHozAlign: "center", hozAlign: "left", vertAlign: "middle", visible: true, minWidth: 80 },
        { title: "Tên đơn vị", field: "tenDonVi", formatter: 'textarea', headerHozAlign: "center", hozAlign: "left", vertAlign: "middle", width: 210, minWidth: 300 },
        { title: "Địa chỉ", field: "diaChi", formatter: 'textarea', headerHozAlign: "center", hozAlign: "left", vertAlign: "middle",  minWidth: 400 },
        { title: "Số điện thoại", field: "soDT", formatter: 'textarea', headerHozAlign: "center", hozAlign: "left", vertAlign: "middle", minWidth: 120 },
        { title: "Email", field: "Email", hozAlign: "left", headerHozAlign: "center", formatter: "textarea", vertAlign: "middle", minWidth: 200 },
        { title: "Website", field: "web", hozAlign: "left", headerHozAlign: "center", formatter: "textarea", vertAlign: "middle", minWidth: 250 },
        { title: "Tên phần mềm viết tắt", field: "TenPMVietTat", hozAlign: "left", headerHozAlign: "center", formatter: "textarea", vertAlign: "middle", minWidth: 200 },
       /* { title: "Phiên bản", field: "PhienBan", hozAlign: "left", headerHozAlign: "center", formatter: "textarea", vertAlign: "middle", minWidth: 300 },*/
        { title: "Đang sử dụng", field: "DangSD", headerWordWrap: true, hozAlign: "center", vertAlign: "middle", formatter: fmDangSD, headerSort: false, width: 135, headerHozAlign: "center" }
    ],
    locale: true,
...
table.on("rowDblClick", function (e, row) {
    $('#maCauHinhID').val(row.getData().maCauHinhID);
    SuaDuLieu(row.getData().maCauHinhID);
});
...
$(document).on('click', '.btnXemAnh', function () {
    $('#mdXemAnh').modal('show');
    $('#listBanner').html("");
    var banner = $(this).attr('data');
    var dsbanner = "";
    if (banner != null && banner.length > 0) {
        ...
        for (let p = 0; p < arrFile.length - 1; p++) {
            if (arrFile[p].lastIndexOf('.') != -1) {
                dsbanner += `<div class="col-3" style="margin-top:10px">
                            <a data-fslightbox="gallery" href="${arrFile[p].replace('~', '')}">
                                <!-- Photo -->
                                <div class="img-responsive img-responsive-1x1 rounded-3 border" style="background-image: url(${arrFile[p].replace('~', '')})"></div>
                            </a>
                        </div>`
...
function ThemDuLieu() {
	...
	$('#tieuDeModal').text("Thêm mới thông tin thiết lập website");
	CKEDITOR.instances.PhienBan.setData("");
    resetForm('#mdThemMoi');
    resetUploadFile_TaiLieu();
    CKEDITOR.instances.PhienBan.setData('');
    $('#DangSD').prop('checked', true);
    UpdateLabelDangSD('#DangSD');
    const MaTuTang = NTS.getAjax("/DanhMuc/DungChung/LayMaTuTang", { strKyTu: '', strCotTang: 'maCauHinh', strBangTang: 'CauHinhWebSite', strDinhDang: '000' });
    if (!MaTuTang.Err) {
        $('#maCauHinh').value(MaTuTang.Result);
    }
    tempthem = "them";
}
var editor = CKEDITOR.replace('PhienBan', {
	toolbar: [],
...
function SuaDuLieu(ID) {
    ...
    $('#maCauHinh').attr('disabled', 'disabled');
    $("#fileVB").val(null);
    $('#tieuDeModal').text('Cập nhật thông tin thiết lập website');

    const result = NTS.getAjax('/HeThong/ThietLapWebSite/LoadDuLieuSua', { id: ID });
    resetUploadFile_TaiLieu();

    if (!result.Err && result.Result != null) {
		let data = result.Result[0];
        $('#maCauHinh').value(data.maCauHinh);
        $('#tenDonVi').value(data.tenDonVi);
        $('#diaChi').value(data.diaChi);
        $('#soDT').value(data.soDT);
        $('#Fax').value(data.Fax);
        $('#Email').value(data.Email);
        //$('#TenDinhKem').value(result.banner);
        $('#web').value(data.web);
        $('#DangSD').value(data.DangSD);
        UpdateLabelDangSD('#DangSD');
        $('#maCauHinhID').val(data.maCauHinhID);
        $('#TenPMVietTat').val(data.TenPMVietTat);
        CKEDITOR.instances.PhienBan.setData(data.PhienBan);
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'maCauHinhID', ID: ID, TenBangHienTai: 'CauHinhWebSite', CacBangKhongXet: [] });

    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/HeThong/ThietLapWebSite/XoaCauHinhWebSite', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
...
$('#btnLuuVaDong').on('click', function () {
    ...
    param[7] = duongDan;
    param[8] = $('#web').value();
    param[9] = $('#DangSD').value();
    param[10] = $('#maCauHinhID').val();
    param[11] = $('#TenPMVietTat').value();
    param[12] = CKEDITOR.instances.PhienBan.getData();

    const result = NTS.getAjax("/HeThong/ThietLapWebSite/LuuThongTin", { data: param });
    debugger
    if (!result.Err) {
        LoadDataTable();
        NTS.thanhcong(result.Msg);
...
$('#btnExport').on('click', async function () {
    ...
    const filteredData = data.filter(
		...
		item => (item.maCauHinh == null ? "" : item.maCauHinh.toLowerCase().indexOf(dulieuloc.toLowerCase()) !== -1)
			|| (item.tenDonVi == null ? "" : item.tenDonVi.toLowerCase().indexOf(dulieuloc.toLowerCase()) !== -1)
			|| (item.diaChi == null ? "" : item.diaChi.toLowerCase().indexOf(dulieuloc.toLowerCase()) !== -1)
			|| (item.soDT == null ? "" : item.soDT.toLowerCase().indexOf(dulieuloc.toLowerCase()) !== -1)
			|| (item.Email == null ? "" : item.Email.toLowerCase().indexOf(dulieuloc.toLowerCase()) !== -1)
			|| (item.TenPMVietTat == null ? "" : item.TenPMVietTat.toLowerCase().indexOf(dulieuloc.toLowerCase()) !== -1)
			|| (item.DangSD == null ? "" : item.DangSD.toLowerCase().indexOf(dulieuloc.toLowerCase()) !== -1)
    );
    ...
    var TenCot = [
		{ 'datafil': 'maCauHinh', 'TenCot': 'Mã', 'DoRong': '10', 'CanhLe': 'Left' },
		{ 'datafil': 'tenDonVi', 'TenCot': 'Tên thiết lập website', 'DoRong': '40', 'CanhLe': 'Left' },
		{ 'datafil': 'diaChi', 'TenCot': 'Địa chỉ', 'DoRong': '40', 'CanhLe': 'Left' },
		{ 'datafil': 'soDT', 'TenCot': 'Số điện thoại', 'DoRong': '30', 'CanhLe': 'Left' },
		{ 'datafil': 'Email', 'TenCot': 'Email', 'DoRong': '30', 'CanhLe': 'Left' },
		{ 'datafil': 'TenPMVietTat', 'TenCot': 'Tên viết tắt', 'DoRong': '30', 'CanhLe': 'Left' },
        { 'datafil': 'DangSD', 'TenCot': 'Trạng thái sử dụng', 'DoRong': '17', 'CanhLe': 'Center' }]
    var result = await NTS.getAjaxAsync('/QuanLy/DungChung/XuatDataExcel', { Cot: JSON.stringify(TenCot), Data: chuoidulieu, TenFile: 'Danh sách thiết lập website' });
    if (result != "") {
        window.open(result);
    }
});
...
$(document).on('click', '.delete-file-attachments', function () {
	...
	debugger
    CanhBaoXoaDinhKem(() => {
		let result = NTS.getAjax('/HeThong/ThietLapWebSite/XoaDinhKem', { ID: id, duongDan: duongDan.attr('data-url-file'), bangDk: bang, cotDk: cot, loai: '' });
        if (!result.Err) {
            duongDan.parent('div').remove();
            //if ($('#mdThemMoi').hasClass('show')) {
            //    LoadGridTaiLieu_UC($('#hdfCapPhatTheID').value());
            //}
            NTS.thanhcong(result.Msg);
        }
        else {
