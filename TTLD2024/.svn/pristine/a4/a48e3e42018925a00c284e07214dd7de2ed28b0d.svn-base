...
var mangGiaTriDau = [];
var mangGiaTriSau = [];
var tenBangThamChieu = 'CauLaoDongBienDong';  // sử dụng cho thiết lập tùy chỉnh hiển thị cột trên lưới chính
var CoThuThapBanDau = false;
var CauLaoDongBanDauID = '';
...
async function LoadDataTable() {
    ...
    Grid1.clearData();
    const GetAll = await NTS.getAjaxAsync("/QuanLy/ThuThapCauLaoDongBienDong/GetAll", { data: saveData });
    if (!GetAll.Err) {
...
// Kiểm tra số lượn phiếu thu thập chưa hoàn thành
async function CanhBaoDuLieuChuaHoanThanh() {
    ...
    saveData[5] = $('#TuNgay_TimKiem_US').value();
    saveData[6] = $('#DenNgay_TimKiem_US').value();
    const result = await NTS.getAjaxAsync("/QuanLy/ThuThapCauLaoDongBienDong/CanhBaoDuLieuChuaHoanThanh", { data: saveData });
    if (!result.Err) {
        if (result.Result > 0) {
...
function LoadComBoToChuc(TinhID, HuyenID, XaID, ThonID) {
    ...
    saveData[2] = XaID;
    saveData[3] = ThonID;
    const kq = NTS.getAjax('/DanhMuc/DungChung/GetToChuc_ComboDaThuThap', { data: saveData }).Result;
...
//Sự kiện chọn tổ chức từ combo
// Trường hợp 1: Tổ chức đã thu thập ban đầu
// Trường hợp 2: Tổ chức chưa thu thập ban đầu
...
$(document).on('click', '.itemToChuc', function () {
    ...
    if (action == "chon") {
        ...
        const result = NTS.getAjax('/QuanLy/ThuThapCauLaoDongBienDong/LoadDuLieuCauLaoDongTheoMaSoThue', { id: value });
        if (!result.Err && result.Result != null) {
            if (result.Result != "") { // Trường hợp 1
                let data = result.Result[0];
                ...
                //LayDuLieuCauLaoDong_us_Buoc3(data.CauLaoDongID);
                CauLaoDongBanDauID = data.CauLaoDongID;
                CoThuThapBanDau = true;
                ...
                NTS.canhbao('Tổ chức này chưa được thu thập thông tin Nhu cầu tuyển dụng lao động của người sử dụng lao động!');
            }
        }
...
// Load tổ chức cho modal chọn tổ chức
...
//Lấy mã số thuế để lấy ra thông tin tổ chức
// Trường hợp 1: Nhập vào mã số thuế thì load lên tổ chức đã thu thập ban đầu
// Trường hợp 2: Nhập vào mã số thuế thì load lên tổ chức đã tồn tại trong hệ thông nhưng chưa thu thập ban đầu
// Trường hợp 3: Nhập vào mã số thuế của tổ chức không tồn tại trong hệ thống

function LoadThongTinToChucTheoMaSoThue(value) {
    const result = NTS.getAjax('/QuanLy/ThuThapCauLaoDongBienDong/LoadDuLieuToChucTheoMaSoThue', { value: value });
    if (result.Result.length != 0) {
        if (!result.Err && result.Result != null) {
            ...
            if (data.GiaTri == '1') {
                ...
                if (data.DiaBanHCID_Tinh == "" || data.DiaBanHCID_Tinh == null) {// Trường hợp 1
                    const result = NTS.getAjax('/QuanLy/ThuThapCauLaoDongBienDong/LoadDuLieuToChucTheoMaSoThue_ChuaThuThap', { value: value });
                    if (result.Result.length != 0) {
                        if (!result.Err && result.Result != null) {
                            let data = result.Result[0];
                            // trường hợp có tồn tại tổ chức với mã số thuế nhập vào
                            $('#ToChucID').value(data.ToChucID);
                            $('#TenKCN_CLD_CLD').value(data.TenKCN);
                            $('#MaSoThue_us_CLD').value(data.MaSoThue);
                            $('#MaToChucUS_CLD').value(data.MaToChuc);
        ...
        case 120:
            if (hotKey == 1) {
                ...
                if ($('#mdThemMoiThuThapCau_us').hasClass('show') && $('#mdThemMoiThuThapCau_us').css('display') === 'block') {
                    console.log('Modal 1 is visible');
                ...
                // Kiểm tra modal 2
                if ($('#mdThemMoiThuThapCau_Buoc2_us').hasClass('show') && $('#mdThemMoiThuThapCau_Buoc2_us').css('display') === 'block') {
                    console.log('Modal 2 is visible');
            ...
            if (hotKey2 == 1) {
                // Kiểm tra modal 3
                if ($('#mdThemMoiThuThapCau_Buoc3_us').hasClass('show') && $('#mdThemMoiThuThapCau_Buoc3_us').css('display') === 'block') {
                    console.log('Modal 3 is visible');
                    $('#btnKetThuc_us').trigger('click');
                    e.preventDefault();
                    break;
                }
                // Nếu không có modal nào được hiển thị
                console.log('No modals are visible');
            }
    }
});
...
$(document).on('click', '#btnTiepTuc_PhieuThuThap', function () {
    ...
    //Trường hợp 1: CoThuThapBanDau == fase => trường hợp thêm mới phiếu thu thập mà tổ chức này chưa được thu thập ban đầu mà thu thập thẳng biến động luôn 
        // Lưu thông tin từng bước của modal thu thập cầu lao động
    // Trường hợp 2: CoThuThapBanDau == fase => chọn lại tổ chức đã thu thập ban đầu, load thông tin ban đầu lên modal r tiến hành thu thập biến động.
    if (CoThuThapBanDau == false) {
        //Trường hợp 1
        if (mangGiaTriSau.length > 0 && mangGiaTriDau.length > 0) {
            ...
            if (different) { // thay đổi thông tin tổ chức
                ...
                CanhBaoThayDoiTTToChuc(() => {
                    var result = NTS.getAjax('/QuanLy/ThuThapCauLaoDongBienDong/LuuThongTinBuoc1', { data: saveData });
                    if (!result.Err) {
                        LoadDataTable();
                        NTS.thanhcong(result.Msg);
        ...
        if (mangGiaTriSau.length > 0 && mangGiaTriDau.length > 0) {
            ...
            if (different) { // thay đổi thông tin tổ chức
                ...
                saveData[29] = $('#ViDo').value();
                saveData[30] = tempthemToChuc;
                saveData[31] = CauLaoDongBanDauID;
                ...
                saveData[32] = $('#CauLaoDongID').value();
                saveData[33] = $('#txtDinhKem_VanBan_US').value();
                saveData[34] = $('#txtDuongDanFileChuKy').value();
                saveData[35] = $('#KyTen_us').value();
                saveData[36] = $('#KinhDo').value();
                saveData[37] = $('#ViDo').value();
                CanhBaoThayDoiTTToChuc(() => {
                    var result = NTS.getAjax('/QuanLy/ThuThapCauLaoDongBienDong/LuuThongTinCauBanDauSangBienDong', { data: saveData });
                    if (!result.Err) {
                        LoadDataTable();
                        NTS.thanhcong(result.Msg);
                        $('#mdThemMoiThuThapCau_us').modal('hide');
...
function XoaDuLieu(ID) {
    ...
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/QuanLy/ThuThapCauLaoDongBienDong/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
...
function SuaDuLieuCauLaoDong_us(ID) {
    ...
    const result = NTS.getAjax('/QuanLy/ThuThapCauLaoDongBienDong/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        PhanQuyenComBoDiaBan('TinhID_us_CLD', 'HuyenID_us_CLD', 'XaID_us_CLD', 'ThonID_us_CLD');
        $('#ToChucID').value(data.ToChucID);
    ...
    editor.on('change', function () {
        ...
        if (valthutu == '4.1') {
            ...
            if (row4) {
                ...
                NTS.getAjax("/QuanLy/ThuThapCauLaoDongBienDong/LuuThongTin_CauLDBanDauTTLD_LaoDong", { id: row4ID, tenCot: 'SoNguoi', soNguoi: inputValue });
            }
        }
        ...
        if (valthutu == '4.2' || valthutu == '4.3' || valthutu == '4.4' || valthutu == '4.5' || valthutu == '4.6' || valthutu == '4.7') {
            ...
            if (row41) {
                ...
                if (warningMessage) {
                    ...
                    NTS.getAjax("/QuanLy/ThuThapCauLaoDongBienDong/LuuThongTin_CauLDBanDauTTLD_LaoDong", { id: rowID, tenCot: 'SoNguoi', soNguoi: inputValue });
                }
            }
        }
            ...
            if (mangKT.includes(valthutu)) {
                ...
                if (sum > maxAllowed) {
                    ...
                    if (row6) {
                        ...
                        NTS.getAjax("/QuanLy/ThuThapCauLaoDongBienDong/LuuThongTin_CauLDBanDauTTLD_LaoDong", { id: row6ID, tenCot: 'SoNguoi', soNguoi: sum });
                    }
                } else {
        ...
        NTS.getAjax("/QuanLy/ThuThapCauLaoDongBienDong/LuuThongTin_CauLDBanDauTTLD_LaoDong", { id: ID, tenCot: 'SoNguoi', soNguoi: inputValue });
...
async function LoadMauThongTinNguoiLaoDongDangLV(ID) {
    ...
    const GetAll = await NTS.getAjaxAsync("/QuanLy/ThuThapCauLaoDongBienDong/LayDuLieuMauNguoiLaoDong", { id: ID });
    if (!GetAll.Err) {
...
function SuaDuLieuCauLaoDong_us_Buoc3(ID) {
    ...
    $('#lblTieuDeThemMoi1_us').text('Cập nhật thông tin Nhu cầu tuyển dụng lao động của người sử dụng lao động');
    resetForm("#mdThemMoiThuThapCau_Buoc3_us");
    const result = NTS.getAjax('/QuanLy/ThuThapCauLaoDongBienDong/LoadDuLieuSua_Buoc3', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#ToChucID').value(data.ToChucID);
        $('#KyTen_us').value(data.NguoiCungCapTT);
...
var SoLuongNguoi = function (cell, onRendered, success, cancel, editorParams) {
    ...
    editor.on('change', function () {
        ...
        // Gọi hàm NTS.getAjax để lưu giá trị mới vào cơ sở dữ liệu
        NTS.getAjax("/QuanLy/ThuThapCauLaoDongBienDong/LuuThongTin_CauLDBanDauTD_NhuCauTuyenDung", { id: ID, tenCot: 'TongSo', soNguoi: inputValue });
        success(inputValue); // Cập nhật giá trị mới cho ô
        //luoi.redraw(true); // Vẽ lại bảng để hiển thị giá trị mới
    });
...
var SoLuongLDNu = function (cell, onRendered, success, cancel, editorParams) {
    ...
    editor.on('change', function () {
        ...
        // Gọi hàm NTS.getAjax để lưu giá trị mới vào cơ sở dữ liệu
        NTS.getAjax("/QuanLy/ThuThapCauLaoDongBienDong/LuuThongTin_CauLDBanDauTD_NhuCauTuyenDung", { id: ID, tenCot: 'SoLuongNu', soNguoi: inputValueInt });
...
async function LoadMauThongTinNhuCauTuyenDung(ID) {
    ...
    const GetAll = await NTS.getAjaxAsync("/QuanLy/ThuThapCauLaoDongBienDong/LayDuLieuMauTuyenDung", { id: ID });
    if (!GetAll.Err) {
...
$(document).on('click', '#btnKetThuc_us', function () {
    ...
    var result = NTS.getAjax('/QuanLy/ThuThapCauLaoDongBienDong/LuuThongTinBuoc3', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
        NTS.thanhcong(result.Msg);
        $('#mdThemMoiThuThapCau_Buoc3_us').modal('hide');
...
async function XuatMau02_TT01(ID) {
    var data = await NTS.getAjaxAsync('/QuanLy/ThuThapCauLaoDongBienDong/XuatMau02', { id: ID });
    //window.open(data);
    try {
...
$('#btnExport').on('click', async function () {
    ...
    saveData[6] = $('#DenNgay_TimKiem_US').value();
    Grid1.clearData();
    var kq = await NTS.getAjax('/QuanLy/ThuThapCauLaoDongBienDong/XuatExcel_DSThuThapCauLaoDongBienDong', { data: saveData });
    if (!kq.Err) {
        window.open(kq);
    } else {
        NTS.loi(kq.Msg);
