...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'LinhVucDaoTao', 'LinhVucDaoTaoID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().LinhVucDaoTaoID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().LinhVucDaoTaoID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().LinhVucDaoTaoID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaLinhVucDaoTao", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên lĩnh vực đào tạo giáo dục nghề nghiệp", field: "TenLinhVucDaoTao", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "LinhVucDaoTaoID", field: "LinhVucDaoTaoID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/LinhVucDaoTao/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'LinhVucDaoTao', 'LinhVucDaoTaoID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới lĩnh vực đào tạo giáo dục nghề nghiệp');
    $('#mdThemMoi').modal('show');
    $('#MaLinhVucDaoTao').value('');
    $('#TenLinhVucDaoTao').value('');
    $('#LinhVucDaoTaoID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#LinhVucDaoTaoID').val(row.getData().LinhVucDaoTaoID);
    SuaDuLieu(row.getData().LinhVucDaoTaoID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật lĩnh vực đào tạo giáo dục nghề nghiệp');
    const result = NTS.getAjax('/DanhMuc/LinhVucDaoTao/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#LinhVucDaoTaoID').value(data.LinhVucDaoTaoID);
        $('#MaLinhVucDaoTao').value(data.MaLinhVucDaoTao);
        $('#TenLinhVucDaoTao').value(data.TenLinhVucDaoTao);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraNhapSo($('#MaLinhVucDaoTao').value())) {
        return NTS.canhbao("Mã chỉ được chứa ký tự số!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#LinhVucDaoTaoID').value();
    saveData[2] = $('#MaLinhVucDaoTao').value();
    saveData[3] = $('#TenLinhVucDaoTao').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/LinhVucDaoTao/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#LinhVucDaoTaoID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'LinhVucDaoTaoID', ID: ID, TenBangHienTai: 'LinhVucDaoTao', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/LinhVucDaoTao/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
