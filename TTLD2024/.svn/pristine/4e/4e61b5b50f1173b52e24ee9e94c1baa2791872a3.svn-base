...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'PhanLoaiHo', 'PhanLoaiHoID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().PhanLoaiHoID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().PhanLoaiHoID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().PhanLoaiHoID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaPhanLoaiHo", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên phân loại hộ gia đình", field: "TenPhanLoaiHo", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "PhanLoaiHoID", field: "PhanLoaiHoID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/PhanLoaiHo/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'PhanLoaiHo', 'PhanLoaiHoID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin phân loại hộ gia đình');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#PhanLoaiHoID').val(row.getData().PhanLoaiHoID);
    SuaDuLieu(row.getData().PhanLoaiHoID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin phân loại hộ gia đình');
    const result = NTS.getAjax('/DanhMuc/PhanLoaiHo/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#PhanLoaiHoID').value(data.PhanLoaiHoID);
        $('#MaPhanLoaiHo').value(data.MaPhanLoaiHo);
        $('#TenPhanLoaiHo').value(data.TenPhanLoaiHo);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraNhapSo($('#MaPhanLoaiHo').value())) {
    ...
    saveData[0] = tempthem;
    saveData[1] = $('#PhanLoaiHoID').value();
    saveData[2] = $('#MaPhanLoaiHo').value();
    saveData[3] = $('#TenPhanLoaiHo').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/PhanLoaiHo/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#PhanLoaiHoID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'PhanLoaiHoID', ID: ID, TenBangHienTai: 'PhanLoaiHo', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/PhanLoaiHo/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
