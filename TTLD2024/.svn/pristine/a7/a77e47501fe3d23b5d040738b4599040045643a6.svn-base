...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'YeuCauKinhNghiem', 'YeuCauKinhNghiemID');

});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().YeuCauKinhNghiemID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().YeuCauKinhNghiemID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().YeuCauKinhNghiemID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        //{ title: "Mã", field: "MaYeuCauKinhNghiem", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Mã", field: "MaYeuCauKinhNghiem", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên yêu cầu kinh nghiệm", field: "TenYeuCauKinhNghiem", formatter: 'textarea', hozAlign: "left", width: 400, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "YeuCauKinhNghiemID", field: "YeuCauKinhNghiemID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/YeuCauKinhNghiem/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'YeuCauKinhNghiem', 'YeuCauKinhNghiemID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#MaYeuCauKinhNghiem').prop('disabled', false);
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin yêu cầu kinh nghiệm');
    $('#mdThemMoi').modal('show');
    $('#MaYeuCauKinhNghiem').value('');
    $('#TenYeuCauKinhNghiem').value('');
    $('#YeuCauKinhNghiemID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#YeuCauKinhNghiemID').val(row.getData().YeuCauKinhNghiemID);
    SuaDuLieu(row.getData().YeuCauKinhNghiemID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin yêu cầu kinh nghiệm');
    const result = NTS.getAjax('/DanhMuc/YeuCauKinhNghiem/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#YeuCauKinhNghiemID').value(data.YeuCauKinhNghiemID);
        $('#MaYeuCauKinhNghiem').value(data.MaYeuCauKinhNghiem);
        $('#TenYeuCauKinhNghiem').value(data.TenYeuCauKinhNghiem);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    saveData[0] = tempthem;
    saveData[1] = $('#YeuCauKinhNghiemID').value();
    saveData[2] = $('#MaYeuCauKinhNghiem').value();
    saveData[3] = $('#TenYeuCauKinhNghiem').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/YeuCauKinhNghiem/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#YeuCauKinhNghiemID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'YeuCauKinhNghiemID', ID: ID, TenBangHienTai: 'YeuCauKinhNghiem', CacBangKhongXet: [] });
    //console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/YeuCauKinhNghiem/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
