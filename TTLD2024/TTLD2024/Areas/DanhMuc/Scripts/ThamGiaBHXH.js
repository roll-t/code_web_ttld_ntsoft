...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'ThamGiaBHXH', 'ThamGiaBHXHID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().ThamGiaBHXHID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().ThamGiaBHXHID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().ThamGiaBHXHID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaThamGiaBHXH", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên hình thức tham gia bảo hiểm xã hội", field: "TenThamGiaBHXH", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "ThamGiaBHXHID", field: "ThamGiaBHXHID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/ThamGiaBHXH/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'ThamGiaBHXH', 'ThamGiaBHXHID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin tham gia bảo hiểm xã hội');
    $('#mdThemMoi').modal('show');
    $('#MaThamGiaBHXH').value('');
    $('#TenThamGiaBHXH').value('');
    $('#ThamGiaBHXHID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#ThamGiaBHXHID').val(row.getData().ThamGiaBHXHID);
    SuaDuLieu(row.getData().ThamGiaBHXHID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin tham gia bảo hiểm xã hội');
    const result = NTS.getAjax('/DanhMuc/ThamGiaBHXH/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#ThamGiaBHXHID').value(data.ThamGiaBHXHID);
        $('#MaThamGiaBHXH').value(data.MaThamGiaBHXH);
        $('#TenThamGiaBHXH').value(data.TenThamGiaBHXH);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraNhapSo($('#MaThamGiaBHXH').value())) {
    ...
    saveData[0] = tempthem;
    saveData[1] = $('#ThamGiaBHXHID').value();
    saveData[2] = $('#MaThamGiaBHXH').value();
    saveData[3] = $('#TenThamGiaBHXH').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/ThamGiaBHXH/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#ThamGiaBHXHID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'ThamGiaBHXHID', ID: ID, TenBangHienTai: 'ThamGiaBHXH', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/ThamGiaBHXH/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
