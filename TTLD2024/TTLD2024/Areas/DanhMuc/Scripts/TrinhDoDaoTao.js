...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'TrinhDoDaoTao', 'TrinhDoDaoTaoID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().TrinhDoDaoTaoID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().TrinhDoDaoTaoID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().TrinhDoDaoTaoID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaTrinhDoDaoTao", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên trình độ đào tạo giáo dục nghề nghiệp", field: "TenTrinhDoDaoTao", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "TrinhDoDaoTaoID", field: "TrinhDoDaoTaoID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/TrinhDoDaoTao/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'TrinhDoDaoTao', 'TrinhDoDaoTaoID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới trình độ đào tạo giáo dục nghề nghiệp');
    $('#mdThemMoi').modal('show');
    $('#MaTrinhDoDaoTao').value('');
    $('#TenTrinhDoDaoTao').value('');
    $('#TrinhDoDaoTaoID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#TrinhDoDaoTaoID').val(row.getData().TrinhDoDaoTaoID);
    SuaDuLieu(row.getData().TrinhDoDaoTaoID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật trình độ đào tạo giáo dục nghề nghiệp');
    const result = NTS.getAjax('/DanhMuc/TrinhDoDaoTao/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#TrinhDoDaoTaoID').value(data.TrinhDoDaoTaoID);
        $('#MaTrinhDoDaoTao').value(data.MaTrinhDoDaoTao);
        $('#TenTrinhDoDaoTao').value(data.TenTrinhDoDaoTao);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    saveData[0] = tempthem;
    saveData[1] = $('#TrinhDoDaoTaoID').value();
    saveData[2] = $('#MaTrinhDoDaoTao').value();
    saveData[3] = $('#TenTrinhDoDaoTao').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/TrinhDoDaoTao/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#TrinhDoDaoTaoID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'TrinhDoDaoTaoID', ID: ID, TenBangHienTai: 'TrinhDoDaoTao', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/TrinhDoDaoTao/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
