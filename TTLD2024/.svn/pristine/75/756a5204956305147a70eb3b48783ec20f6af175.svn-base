...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'KhoLuuTru', 'KhoLuuTruID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().KhoLuuTruID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().KhoLuuTruID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().KhoLuuTruID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaKhoLuuTru", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên kho lưu trữ", field: "TenKhoLuuTru", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "KhoLuuTruID", field: "KhoLuuTruID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/KhoLuuTru/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'KhoLuuTru', 'KhoLuuTruID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin kho lưu trữ');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#KhoLuuTruID').val(row.getData().KhoLuuTruID);
    SuaDuLieu(row.getData().KhoLuuTruID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin kho lưu trữ');
    const result = NTS.getAjax('/DanhMuc/KhoLuuTru/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#KhoLuuTruID').value(data.KhoLuuTruID);
        $('#MaKhoLuuTru').value(data.MaKhoLuuTru);
        $('#TenKhoLuuTru').value(data.TenKhoLuuTru);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraNhapSo($('#MaKhoLuuTru').value())) {
        return NTS.canhbao("Mã chỉ được chứa ký tự số!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#KhoLuuTruID').value();
    saveData[2] = $('#MaKhoLuuTru').value();
    saveData[3] = $('#TenKhoLuuTru').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/KhoLuuTru/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#KhoLuuTruID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'KhoLuuTruID', ID: ID, TenBangHienTai: 'KhoLuuTru', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/KhoLuuTru/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
