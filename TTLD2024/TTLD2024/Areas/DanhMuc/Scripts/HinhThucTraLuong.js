...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'HinhThucTraLuong', 'HinhThucTraLuongID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().HinhThucTraLuongID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().HinhThucTraLuongID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().HinhThucTraLuongID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaHinhThucTraLuong", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên hình thức trả lương", field: "TenHinhThucTraLuong", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "HinhThucTraLuongID", field: "HinhThucTraLuongID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/HinhThucTraLuong/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'HinhThucTraLuong', 'HinhThucTraLuongID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới hình thức trả lương');
    $('#mdThemMoi').modal('show');
    $('#MaHinhThucTraLuong').value('');
    $('#TenHinhThucTraLuong').value('');
    $('#HinhThucTraLuongID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#HinhThucTraLuongID').val(row.getData().HinhThucTraLuongID);
    SuaDuLieu(row.getData().HinhThucTraLuongID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật hình thức trả lương');
    const result = NTS.getAjax('/DanhMuc/HinhThucTraLuong/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#HinhThucTraLuongID').value(data.HinhThucTraLuongID);
        $('#MaHinhThucTraLuong').value(data.MaHinhThucTraLuong);
        $('#TenHinhThucTraLuong').value(data.TenHinhThucTraLuong);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraKhongCoKyTuDacBiet($('#MaHinhThucTraLuong').value())) {
        return NTS.canhbao("Mã không chứa ký tự đăc biệt!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#HinhThucTraLuongID').value();
    saveData[2] = $('#MaHinhThucTraLuong').value();
    saveData[3] = $('#TenHinhThucTraLuong').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/HinhThucTraLuong/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#HinhThucTraLuongID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'HinhThucTraLuongID', ID: ID, TenBangHienTai: 'HinhThucTraLuong', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/HinhThucTraLuong/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
