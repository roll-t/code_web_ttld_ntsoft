...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'ThoiGianThatNghiep', 'ThoiGianThatNghiepID');
});



var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().ThoiGianThatNghiepID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().ThoiGianThatNghiepID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().ThoiGianThatNghiepID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaThoiGianThatNghiep", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên thời gian thất nghiệp", field: "TenThoiGianThatNghiep", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "ThoiGianThatNghiepID", field: "ThoiGianThatNghiepID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/ThoiGianThatNghiep/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'ThoiGianThatNghiep', 'ThoiGianThatNghiepID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới thời gian thất nghiệp');
    $('#mdThemMoi').modal('show');
    $('#MaThoiGianThatNghiep').value('');
    $('#TenThoiGianThatNghiep').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#ThoiGianThatNghiepID').val(row.getData().ThoiGianThatNghiepID);
    SuaDuLieu(row.getData().ThoiGianThatNghiepID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thời gian thất nghiệp');
    const result = NTS.getAjax('/DanhMuc/ThoiGianThatNghiep/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#ThoiGianThatNghiepID').value(data.ThoiGianThatNghiepID);
        $('#MaThoiGianThatNghiep').value(data.MaThoiGianThatNghiep);
        $('#TenThoiGianThatNghiep').value(data.TenThoiGianThatNghiep);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraKhongCoKyTuDacBiet($('#MaThoiGianThatNghiep').value())) {
        return NTS.canhbao("Mã không được chứa ký tự đăc biệt!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#ThoiGianThatNghiepID').value();
    saveData[2] = $('#MaThoiGianThatNghiep').value();
    saveData[3] = $('#TenThoiGianThatNghiep').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/ThoiGianThatNghiep/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#ThoiGianThatNghiepID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'ThoiGianThatNghiepID', ID: ID, TenBangHienTai: 'ThoiGianThatNghiep', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/ThoiGianThatNghiep/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
