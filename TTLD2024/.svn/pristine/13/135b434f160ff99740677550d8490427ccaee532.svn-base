...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'ThoiHanLuuTru', 'ThoiHanLuuTruID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().ThoiHanLuuTruID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().ThoiHanLuuTruID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().ThoiHanLuuTruID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaThoiHanLuuTru", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên thời hạn lưu trữ", field: "TenThoiHanLuuTru", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "ThoiHanLuuTruID", field: "ThoiHanLuuTruID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/ThoiHanLuuTru/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'ThoiHanLuuTru', 'ThoiHanLuuTruID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin thời hạn lưu trữ');
    $('#mdThemMoi').modal('show');
    $('#MaThoiHanLuuTru').value('');
    $('#TenThoiHanLuuTru').value('');
    $('#ThoiHanLuuTruID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#ThoiHanLuuTruID').val(row.getData().ThoiHanLuuTruID);
    SuaDuLieu(row.getData().ThoiHanLuuTruID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin thời hạn lưu trữ');
    const result = NTS.getAjax('/DanhMuc/ThoiHanLuuTru/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#ThoiHanLuuTruID').value(data.ThoiHanLuuTruID);
        $('#MaThoiHanLuuTru').value(data.MaThoiHanLuuTru);
        $('#TenThoiHanLuuTru').value(data.TenThoiHanLuuTru);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraNhapSo($('#MaThoiHanLuuTru').value())) {
        return NTS.canhbao("Mã chỉ được chứa ký tự số!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#ThoiHanLuuTruID').value();
    saveData[2] = $('#MaThoiHanLuuTru').value();
    saveData[3] = $('#TenThoiHanLuuTru').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/ThoiHanLuuTru/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#ThoiHanLuuTruID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'ThoiHanLuuTruID', ID: ID, TenBangHienTai: 'ThoiHanLuuTru', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/ThoiHanLuuTru/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
