...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'TrinhDoKyNangNghe', 'TrinhDoKyNangNgheID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().TrinhDoKyNangNgheID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().TrinhDoKyNangNgheID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().TrinhDoKyNangNgheID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaTrinhDoKyNangNghe", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên trình độ kỹ năng nghề", field: "TenTrinhDoKyNangNghe", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "TrinhDoKyNangNgheID", field: "TrinhDoKyNangNgheID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/TrinhDoKyNangNghe/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'TrinhDoKyNangNghe', 'TrinhDoKyNangNgheID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới trình độ kỹ năng nghề');
    $('#mdThemMoi').modal('show');
    $('#MaTrinhDoKyNangNghe').value('');
    $('#TenTrinhDoKyNangNghe').value('');
    $('#TrinhDoKyNangNgheID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#TrinhDoKyNangNgheID').val(row.getData().TrinhDoKyNangNgheID);
    SuaDuLieu(row.getData().TrinhDoKyNangNgheID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật trình độ kỹ năng nghề');
    const result = NTS.getAjax('/DanhMuc/TrinhDoKyNangNghe/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#TrinhDoKyNangNgheID').value(data.TrinhDoKyNangNgheID);
        $('#MaTrinhDoKyNangNghe').value(data.MaTrinhDoKyNangNghe);
        $('#TenTrinhDoKyNangNghe').value(data.TenTrinhDoKyNangNghe);
...
$(document).on('input', '#MaTrinhDoKyNangNghe', function (event) {
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    saveData[0] = tempthem;
    saveData[1] = $('#TrinhDoKyNangNgheID').value();
    saveData[2] = $('#MaTrinhDoKyNangNghe').value();
    saveData[3] = $('#TenTrinhDoKyNangNghe').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/TrinhDoKyNangNghe/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#TrinhDoKyNangNgheID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'TrinhDoKyNangNgheID', ID: ID, TenBangHienTai: 'TrinhDoKyNangNghe', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/TrinhDoKyNangNghe/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
