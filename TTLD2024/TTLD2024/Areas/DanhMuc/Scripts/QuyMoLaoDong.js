...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'QuyMoLaoDong', 'QuyMoLaoDongID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().QuyMoLaoDongID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().QuyMoLaoDongID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().QuyMoLaoDongID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaQuyMoLaoDong", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên quy mô lao động", field: "TenQuyMoLaoDong", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "QuyMoLaoDongID", field: "QuyMoLaoDongID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/QuyMoLaoDong/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'QuyMoLaoDong', 'QuyMoLaoDongID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới quy mô lao động');
    $('#mdThemMoi').modal('show');
    $('#MaQuyMoLaoDong').value('');
    $('#TenQuyMoLaoDong').value('');
    $('#QuyMoLaoDongID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#QuyMoLaoDongID').val(row.getData().QuyMoLaoDongID);
    SuaDuLieu(row.getData().QuyMoLaoDongID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật quy mô lao động');
    const result = NTS.getAjax('/DanhMuc/QuyMoLaoDong/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#QuyMoLaoDongID').value(data.QuyMoLaoDongID);
        $('#MaQuyMoLaoDong').value(data.MaQuyMoLaoDong);
        $('#TenQuyMoLaoDong').value(data.TenQuyMoLaoDong);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    //    return false;
    //}
    if (KiemTraKhongCoKyTuDacBiet($('#MaQuyMoLaoDong').value())) {
        return NTS.canhbao("Mã không chứa ký tự đăc biệt!");
    }

    if (isAlphaNumericWithSpecialChars($('#TenQuyMoLaoDong').value())) {
        return NTS.canhbao("Tên quy mô lao động không chứa ký tự đặc biệt!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#QuyMoLaoDongID').value();
    saveData[2] = $('#MaQuyMoLaoDong').value();
    saveData[3] = $('#TenQuyMoLaoDong').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/QuyMoLaoDong/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#QuyMoLaoDongID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'QuyMoLaoDongID', ID: ID, TenBangHienTai: 'QuyMoLaoDong', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/QuyMoLaoDong/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
