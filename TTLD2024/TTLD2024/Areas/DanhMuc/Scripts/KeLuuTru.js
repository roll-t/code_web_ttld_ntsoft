...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'KeLuuTru', 'KeLuuTruID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().KeLuuTruID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().KeLuuTruID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().KeLuuTruID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaKeLuuTru", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên kệ lưu trữ", field: "TenKeLuuTru", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        { title: "Kho lưu trữ", field: "TenKhoLuuTru", formatter: 'textarea', hozAlign: "left", minWidth: 383, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "KeLuuTruID", field: "KeLuuTruID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/KeLuuTru/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'KeLuuTru', 'KeLuuTruID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin kệ lưu trữ');
    ...
    NTS.loadDataCombo({
        name: '#KhoLuuTruID',
        ajaxUrl: '/DanhMuc/KeLuuTru/GetKhoLuuTru_Combo',
        ajaxParam: { id: '' },
        columns: 2,
        indexValue: 0,
        indexText1: 2,
        textShowTatCa: '- Chọn kho lưu trữ -',
        showTatCa: !0
    });
...
Grid1.on("rowDblClick", function (e, row) {
    $('#KeLuuTruID').val(row.getData().KeLuuTruID);
    SuaDuLieu(row.getData().KeLuuTruID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin kệ lưu trữ');
    const result = NTS.getAjax('/DanhMuc/KeLuuTru/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#KeLuuTruID').value(data.KeLuuTruID);
        $('#MaKeLuuTru').value(data.MaKeLuuTru);
        $('#TenKeLuuTru').value(data.TenKeLuuTru);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraNhapSo($('#MaKeLuuTru').value())) {
        return NTS.canhbao("Mã chỉ được chứa ký tự số!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#KeLuuTruID').value();
    saveData[2] = $('#MaKeLuuTru').value();
    saveData[3] = $('#TenKeLuuTru').value();
    saveData[4] = $('#KhoLuuTruID').value();
    saveData[5] = $('#DienGiai').value();
    saveData[6] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/KeLuuTru/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'KeLuuTruID', ID: ID, TenBangHienTai: 'KeLuuTru', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/KeLuuTru/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
