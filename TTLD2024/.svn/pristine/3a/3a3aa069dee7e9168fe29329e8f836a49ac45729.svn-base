...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'ViTriViecLam', 'ViTriViecLamID');
});

function LoadCombo() {
    NTS.loadDataCombo({
        name: '#NhomVTVLID',
        ajaxUrl: '/DanhMuc/DungChung/GetNhomVTVL_Combo',
        columns: 2,
        indexValue: 0,
        indexText: 1,
        indexText1: 2,
...
var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().ViTriViecLamID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().ViTriViecLamID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().ViTriViecLamID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaViTriViecLam", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên vị trí việc làm", field: "TenViTriViecLam", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "ViTriViecLamID", field: "ViTriViecLamID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/ViTriViecLam/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'ViTriViecLam', 'ViTriViecLamID');
...
$(document).on('click', '#btnThemMoi', function () {
    LoadCombo();
    $('#lblTieuDeThemMoi').text('Thêm mới vị trí việc làm');
    $('#mdThemMoi').modal('show');
    $('#MaViTriViecLam').value('');
    $('#TenViTriViecLam').value('');
    $('#NhomVTVLID').value('');
    $('#ViTriViecLamID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#ViTriViecLamID').val(row.getData().ViTriViecLamID);
    SuaDuLieu(row.getData().ViTriViecLamID);
});
...
function SuaDuLieu(ID) {
    ...
    LoadCombo();
    $('#lblTieuDeThemMoi').text('Cập nhật vị trí việc làm');
    const result = NTS.getAjax('/DanhMuc/ViTriViecLam/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#ViTriViecLamID').value(data.ViTriViecLamID);
        $('#MaViTriViecLam').value(data.MaViTriViecLam);
        $('#TenViTriViecLam').value(data.TenViTriViecLam);
        $('#NhomVTVLID').value(data.NhomVTVLID);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraKhongCoKyTuDacBiet($('#MaViTriViecLam').value())) {
        return NTS.canhbao("Mã không được chứa ký tự đăc biệt!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#ViTriViecLamID').value();
    saveData[2] = $('#MaViTriViecLam').value();
    saveData[3] = $('#TenViTriViecLam').value();
    saveData[4] = $('#NhomVTVLID').value();
    saveData[5] = $('#DienGiai').value();
    saveData[6] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/ViTriViecLam/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#ViTriViecLamID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'ViTriViecLamID', ID: ID, TenBangHienTai: 'ViTriViecLam', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/ViTriViecLam/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
