...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'TinhTrangHonNhan', 'TinhTrangHonNhanID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().TinhTrangHonNhanID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().TinhTrangHonNhanID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().TinhTrangHonNhanID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaTinhTrangHonNhan", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên tình trạng hôn nhân", field: "TenTinhTrangHonNhan", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "TinhTrangHonNhanID", field: "TinhTrangHonNhanID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/TinhTrangHonNhan/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'TinhTrangHonNhan', 'TinhTrangHonNhanID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin tình trạng hôn nhân');
    $('#mdThemMoi').modal('show');
    $('#MaTinhTrangHonNhan').value('');
    $('#TenTinhTrangHonNhan').value('');
    $('#TinhTrangHonNhanID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#TinhTrangHonNhanID').val(row.getData().TinhTrangHonNhanID);
    SuaDuLieu(row.getData().TinhTrangHonNhanID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin tình trạng hôn nhân');
    const result = NTS.getAjax('/DanhMuc/TinhTrangHonNhan/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#TinhTrangHonNhanID').value(data.TinhTrangHonNhanID);
        $('#MaTinhTrangHonNhan').value(data.MaTinhTrangHonNhan);
        $('#TenTinhTrangHonNhan').value(data.TenTinhTrangHonNhan);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraNhapSo($('#MaTinhTrangHonNhan').value())) {
    ...
    saveData[0] = tempthem;
    saveData[1] = $('#TinhTrangHonNhanID').value();
    saveData[2] = $('#MaTinhTrangHonNhan').value();
    saveData[3] = $('#TenTinhTrangHonNhan').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/TinhTrangHonNhan/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#TinhTrangHonNhanID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'TinhTrangHonNhanID', ID: ID, TenBangHienTai: 'TinhTrangHonNhan', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/TinhTrangHonNhan/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
