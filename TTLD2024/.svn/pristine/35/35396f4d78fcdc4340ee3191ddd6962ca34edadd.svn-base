...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'CheDoPhucLoi', 'CheDoPhucLoiID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().CheDoPhucLoiID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().CheDoPhucLoiID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().CheDoPhucLoiID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaCheDoPhucLoi", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên chế độ phúc lợi", field: "TenCheDoPhucLoi", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "CheDoPhucLoiID", field: "CheDoPhucLoiID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/CheDoPhucLoi/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'CheDoPhucLoi', 'CheDoPhucLoiID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin chế độ phúc lợi');
    $('#mdThemMoi').modal('show');
    $('#MaCheDoPhucLoi').value('');
    $('#TenCheDoPhucLoi').value('');
    $('#CheDoPhucLoiID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#CheDoPhucLoiID').val(row.getData().CheDoPhucLoiID);
    SuaDuLieu(row.getData().CheDoPhucLoiID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin chế độ phúc lợi');
    const result = NTS.getAjax('/DanhMuc/CheDoPhucLoi/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#CheDoPhucLoiID').value(data.CheDoPhucLoiID);
        $('#MaCheDoPhucLoi').value(data.MaCheDoPhucLoi);
        $('#TenCheDoPhucLoi').value(data.TenCheDoPhucLoi);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraNhapSo($('#MaCheDoPhucLoi').value())) {
        return NTS.canhbao("Mã chỉ được chứa ký tự số!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#CheDoPhucLoiID').value();
    saveData[2] = $('#MaCheDoPhucLoi').value();
    saveData[3] = $('#TenCheDoPhucLoi').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/CheDoPhucLoi/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#CheDoPhucLoiID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'CheDoPhucLoiID', ID: ID, TenBangHienTai: 'CheDoPhucLoi', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/CheDoPhucLoi/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
