...
{
    public class DichVuDKController : Controller
    {
        // GET: DanhMuc/DichVuDK
        ...
        public string GetAll()
        {
            ...
            try
            {
                ...
                DataTable duLieu = SqlHelper.ExecuteDataset(NTSSession.GetConnectionString2(), "Proc_GetAllDichVuDK", null).Tables[0];
        ...
        public string LoadDuLieuSua(string id)
        {
            ...
            try
            {
                ...
                DataTable duLieu = SqlHelper.ExecuteDataset(NTSSession.GetConnectionString1(), "Proc_GetAllDichVuDKByID", para).Tables[0];
                ep.Result = duLieu;
                return JSonHelper.ToJson(ep);
            }
            catch (Exception ex)
        ...
        public string LuuThongTin(object[] data)
        {
            try
            {
                ...
                if (data[0].ToString() == "them" && DungChungController.KiemTraTonTai(data[2].ToString(), "MaDichVuDK", "DichVuDK"))
                {
                    return NTSThongBao.DaTonTaiMa();
                }
                if (data[0].ToString() == "sua" && DungChungController.KiemTraTonTaiSua(data[2].ToString(), "MaDichVuDK", "DichVuDK", "DichVuDKID", data[1].ToString()))
                {
                    return NTSThongBao.DaTonTaiMa();
                }
                ...
                SqlParameter[] para = {
                    ...
                    new SqlParameter("@DichVuDKID", DungChung.NormalizationGuid(data[1].ToString())),
                    new SqlParameter("@MaDichVuDK", data[2].ToString()),
                    new SqlParameter("@TenDichVuDK", data[3].ToString()),
                    new SqlParameter("@DienGiai", data[4].ToString()),
                    new SqlParameter("@TrangThai", DungChung.NormalizationBoolean(data[5].ToString())),
                };
                DataSet duLieu = SqlHelper.ExecuteDataset(NTSSession.GetConnectionString1(), "Proc_LuuThongTinDichVuDK", para);
                if (Loai == "them")
                {
                    duLieu.Tables[0].TableName = "DichVuDK";
        ...
        public string XoaDuLieu(string id)
        {
            try
            {
                ...
                DataSet duLieu = SqlHelper.ExecuteDataset(NTSSession.GetConnectionString1(), "Delete_DichVuDK", para);
                if (duLieu.Tables[0].Rows[0][0].ToString() != "")
                {
                    duLieu.Tables[0].TableName = "DichVuDK";
