...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'MucDichLamViec', 'MucDichLamViecID');

});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().MucDichLamViecID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().MucDichLamViecID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().MucDichLamViecID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        //{ title: "Mã", field: "MaMucDichLamViec", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Mã", field: "MaMucDichLamViec", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên mục đích làm việc", field: "TenMucDichLamViec", formatter: 'textarea', hozAlign: "left", width: 400, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "MucDichLamViecID", field: "MucDichLamViecID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/MucDichLamViec/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'MucDichLamViec', 'MucDichLamViecID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#MaMucDichLamViec').prop('disabled', false);
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin mục đích làm việc');
    $('#mdThemMoi').modal('show');
    $('#MaMucDichLamViec').value('');
    $('#TenMucDichLamViec').value('');
    $('#MucDichLamViecID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#MucDichLamViecID').val(row.getData().MucDichLamViecID);
    SuaDuLieu(row.getData().MucDichLamViecID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin mục đích làm việc');
    const result = NTS.getAjax('/DanhMuc/MucDichLamViec/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#MucDichLamViecID').value(data.MucDichLamViecID);
        $('#MaMucDichLamViec').value(data.MaMucDichLamViec);
        $('#TenMucDichLamViec').value(data.TenMucDichLamViec);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    saveData[0] = tempthem;
    saveData[1] = $('#MucDichLamViecID').value();
    saveData[2] = $('#MaMucDichLamViec').value();
    saveData[3] = $('#TenMucDichLamViec').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/MucDichLamViec/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#MucDichLamViecID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'MucDichLamViecID', ID: ID, TenBangHienTai: 'MucDichLamViec', CacBangKhongXet: [] });
    //console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/MucDichLamViec/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
