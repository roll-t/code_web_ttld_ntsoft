...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'NgheDaoTao', 'NgheDaoTaoID');
});

function LoadCombo() {
    //NTS.loadDataCombo({
    //    name: '#NgheDaoTaoID_Cha',
    //    ajaxUrl: '/DanhMuc/DungChung/GetNgheDaoTao_Combo',
    //    ajaxParam: { id: $('#NgheDaoTaoID').value() },
    //    columns: 2,
    //    indexValue: 0,
    //    indexText: 1,
    //    indexText1: 2,
    ...
    NTS.loadDataCombo({
        name: '#CapID',
        ajaxUrl: '/DanhMuc/DungChung/GetCap_Combo_4',
        indexValue: 0,
        indexText: 1,
        textShowTatCa: '-Chọn-',
        showTatCa: !0
...
$("#CapID").change(function () {
    NTS.loadDataCombo({
        name: '#NgheDaoTaoID_Cha',
        ajaxUrl: '/DanhMuc/DungChung/GetNghedaotao_ComboTheoCap',
        ajaxParam: { data: [$('#NgheDaoTaoID').val(), $(this).val()] },
        columns: 2,
        indexValue: 0,
        indexText: 1,
        indexText1: 2,
...
var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().NgheDaoTaoID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().NgheDaoTaoID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().NgheDaoTaoID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        //{ title: "Mã", field: "MaNgheDaoTao", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        ...
        { title: "Tên nghề đào tạo", field: "TenNgheDaoTao", formatter: 'textarea', hozAlign: "left", width: 324, vertAlign: "middle", headerHozAlign: "center" },
        { title: "Tên tiếng anh", field: "TenTiengAnh", formatter: 'textarea', hozAlign: "left", width: 324, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "NgheDaoTaoID", field: "NgheDaoTaoID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/NgheDaoTao/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'NgheDaoTao', 'NgheDaoTaoID');
...
$(document).on('click', '#btnThemMoi', function () {
    LoadCombo();
    $('#lblTieuDeThemMoi').text('Thêm mới nghề đào tạo');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#NgheDaoTaoID').val(row.getData().NgheDaoTaoID);
    SuaDuLieu(row.getData().NgheDaoTaoID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật nghề đào tạo');
    LoadCombo();
    const result = NTS.getAjax('/DanhMuc/NgheDaoTao/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#mdThemMoi').modal('show');
        $('#NgheDaoTaoID').value(data.NgheDaoTaoID);
        $('#MaNgheDaoTao').value(data.MaNgheDaoTao);
        $('#TenNgheDaoTao').value(data.TenNgheDaoTao);
        $('#TenTiengAnh').value(data.TenTiengAnh);
        $('#CapID').value(data.CapID);
        $('#NgheDaoTaoID_Cha').value(data.NgheDaoTaoID_Cha);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraKhongCoKyTuDacBiet($('#MaNgheDaoTao').value())) {
        return NTS.canhbao("Mã không được chứa ký tự đặc biệt!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#NgheDaoTaoID').value();
    saveData[2] = $('#MaNgheDaoTao').value();
    saveData[3] = $('#TenNgheDaoTao').value();
    saveData[4] = $('#TenTiengAnh').value();
    saveData[5] = $('#NgheDaoTaoID_Cha').value();
    saveData[6] = $('#DienGiai').value();
    saveData[7] = $('#TrangThai').value();
    saveData[8] = $('#CapID').value();
    var result = NTS.getAjax('/DanhMuc/NgheDaoTao/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#NgheDaoTaoID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'NgheDaoTaoID', ID: ID, TenBangHienTai: 'NgheDaoTao', CacBangKhongXet: [] });
    const result_ktxoa1 = NTS.getAjax('/DanhMuc/NgheDaoTao/KiemTraXoa', { ID: ID });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if ((result_ktxoa.Result == null || result_ktxoa.Result == "") && result_ktxoa1.Result[0].KT == 0) {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/NgheDaoTao/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
