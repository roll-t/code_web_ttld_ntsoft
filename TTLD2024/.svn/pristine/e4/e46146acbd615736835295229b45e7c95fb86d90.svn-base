...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'PhongLuuTru', 'PhongLuuTruID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().PhongLuuTruID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().PhongLuuTruID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().PhongLuuTruID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaPhongLuuTru", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên phông lưu trữ", field: "TenPhongLuuTru", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "PhongLuuTruID", field: "PhongLuuTruID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/PhongLuuTru/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'PhongLuuTru', 'PhongLuuTruID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin phông lưu trữ');
    $('#mdThemMoi').modal('show');
    $('#MaPhongLuuTru').value('');
    $('#TenPhongLuuTru').value('');
    $('#PhongLuuTruID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#PhongLuuTruID').val(row.getData().PhongLuuTruID);
    SuaDuLieu(row.getData().PhongLuuTruID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin phông lưu trữ');
    const result = NTS.getAjax('/DanhMuc/PhongLuuTru/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#PhongLuuTruID').value(data.PhongLuuTruID);
        $('#MaPhongLuuTru').value(data.MaPhongLuuTru);
        $('#TenPhongLuuTru').value(data.TenPhongLuuTru);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraNhapSo($('#MaPhongLuuTru').value())) {
        return NTS.canhbao("Mã chỉ được chứa ký tự số!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#PhongLuuTruID').value();
    saveData[2] = $('#MaPhongLuuTru').value();
    saveData[3] = $('#TenPhongLuuTru').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/PhongLuuTru/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#PhongLuuTruID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'PhongLuuTruID', ID: ID, TenBangHienTai: 'PhongLuuTru', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/PhongLuuTru/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
