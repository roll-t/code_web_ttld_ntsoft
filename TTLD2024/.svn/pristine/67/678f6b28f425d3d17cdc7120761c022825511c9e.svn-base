...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'LoaiHopDong', 'LoaiHopDongID');
    //$('#reservationdate').inputmask('dd/mm/yyyy', { 'placeholder': 'dd/mm/yyyy' });
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().LoaiHopDongID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().LoaiHopDongID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().LoaiHopDongID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaLoaiHopDong", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên loại hợp đồng lao động, hợp đồng làm việc", field: "TenLoaiHopDong", formatter: 'textarea', hozAlign: "left", width: 400, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "LoaiHopDongID", field: "LoaiHopDongID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    ...
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/LoaiHopDong/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'LoaiHopDong', 'LoaiHopDongID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới loại hợp đồng lao động, hợp đồng làm việc');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#LoaiHopDongID').val(row.getData().LoaiHopDongID);
    SuaDuLieu(row.getData().LoaiHopDongID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật loại hợp đồng lao động, hợp đồng làm việc');
    const result = NTS.getAjax('/DanhMuc/LoaiHopDong/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#LoaiHopDongID').value(data.LoaiHopDongID);
        $('#MaLoaiHopDong').value(data.MaLoaiHopDong);
        $('#TenLoaiHopDong').value(data.TenLoaiHopDong);
...
$(document).on('input', '#MaLoaiHopDong', function (event) {
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    saveData[0] = tempthem;
    saveData[1] = $('#LoaiHopDongID').value();
    saveData[2] = $('#MaLoaiHopDong').value();
    saveData[3] = $('#TenLoaiHopDong').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/LoaiHopDong/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#LoaiHopDongID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'LoaiHopDongID', ID: ID, TenBangHienTai: 'LoaiHopDong', CacBangKhongXet: [] });
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/LoaiHopDong/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
