...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'DichVuDK', 'DichVuDKID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().DichVuDKID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().DichVuDKID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().DichVuDKID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaDichVuDK", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên dịch vụ đăng ký", field: "TenDichVuDK", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "DichVuDKID", field: "DichVuDKID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/DichVuDK/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'DichVuDK', 'DichVuDKID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới dịch vụ đăng ký');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#DichVuDKID').val(row.getData().DichVuDKID);
    SuaDuLieu(row.getData().DichVuDKID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật dịch vụ đăng ký');
    const result = NTS.getAjax('/DanhMuc/DichVuDK/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#DichVuDKID').value(data.DichVuDKID);
        $('#MaDichVuDK').value(data.MaDichVuDK);
        $('#TenDichVuDK').value(data.TenDichVuDK);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraKhongCoKyTuDacBiet($('#MaDichVuDK').value())) {
        return NTS.canhbao("Mã không được chứa ký tự đăc biệt!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#DichVuDKID').value();
    saveData[2] = $('#MaDichVuDK').value();
    saveData[3] = $('#TenDichVuDK').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/DichVuDK/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'DichVuDKID', ID: ID, TenBangHienTai: 'DichVuDK', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/DichVuDK/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
