...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'DoiTuongUuTien', 'DoiTuongUuTienID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().DoiTuongUuTienID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().DoiTuongUuTienID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().DoiTuongUuTienID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaDoiTuongUuTien", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên đối tượng ưu tiên", field: "TenDoiTuongUuTien", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "DoiTuongUuTienID", field: "DoiTuongUuTienID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/DoiTuongUuTien/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'DoiTuongUuTien', 'DoiTuongUuTienID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin đối tượng ưu tiên');
    $('#mdThemMoi').modal('show');
    $('#MaDoiTuongUuTien').value('');
    $('#TenDoiTuongUuTien').value('');
    $('#DoiTuongUuTienID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#DoiTuongUuTienID').val(row.getData().DoiTuongUuTienID);
    SuaDuLieu(row.getData().DoiTuongUuTienID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin đối tượng ưu tiên');
    const result = NTS.getAjax('/DanhMuc/DoiTuongUuTien/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#DoiTuongUuTienID').value(data.DoiTuongUuTienID);
        $('#MaDoiTuongUuTien').value(data.MaDoiTuongUuTien);
        $('#TenDoiTuongUuTien').value(data.TenDoiTuongUuTien);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraNhapSo($('#MaDoiTuongUuTien').value())) {
        return NTS.canhbao("Mã chỉ được chứa ký tự số!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#DoiTuongUuTienID').value();
    saveData[2] = $('#MaDoiTuongUuTien').value();
    saveData[3] = $('#TenDoiTuongUuTien').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/DoiTuongUuTien/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#DoiTuongUuTienID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'DoiTuongUuTienID', ID: ID, TenBangHienTai: 'DoiTuongUuTien', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/DoiTuongUuTien/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
