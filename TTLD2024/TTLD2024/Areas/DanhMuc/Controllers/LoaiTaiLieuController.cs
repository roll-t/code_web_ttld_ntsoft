    ...
    public class LoaiTaiLieuController : Controller
    {
        ...
        public string GetAll()
        {
            ...
            try
            {
                ...
                DataTable duLieu = SqlHelper.ExecuteDataset(NTSSession.GetConnectionString2(), "Proc_GetAllLoaiTaiLieu", null).Tables[0];
        ...
        public string LuuThongTin(object[] data)
        {
            try
            {
                ...
                if (data[0].ToString() == "them" && DungChungController.KiemTraTonTai(data[2].ToString(), "MaLoaiTaiLieu", "LoaiTaiLieu"))
                {
                    return NTSThongBao.DaTonTaiMa();
                }
                if (data[0].ToString() == "sua" && DungChungController.KiemTraTonTaiSua(data[2].ToString(), "MaLoaiTaiLieu", "LoaiTaiLieu", "LoaiTaiLieuID", data[1].ToString()))
                {
                    return NTSThongBao.DaTonTaiMa();
                }
                ...
                SqlParameter[] para = {
                    ...
                    new SqlParameter("@LoaiTaiLieuID", DungChung.NormalizationGuid(data[1].ToString())),
                    new SqlParameter("@MaLoaiTaiLieu", data[2].ToString()),
                    new SqlParameter("@TenLoaiTaiLieu", data[3].ToString()),
                    new SqlParameter("@DienGiai", data[4].ToString()),
                    new SqlParameter("@TrangThai", DungChung.NormalizationBoolean(data[5].ToString())),
                };
                DataSet duLieu = SqlHelper.ExecuteDataset(NTSSession.GetConnectionString1(), "Proc_LuuThongTinLoaiTaiLieu", para);
                if (Loai == "them")
                {
                    duLieu.Tables[0].TableName = "LoaiTaiLieu";
        ...
        public string LoadDuLieuSua(string id)
        {
            ...
            try
            {
                ...
                DataTable duLieu = SqlHelper.ExecuteDataset(NTSSession.GetConnectionString1(), "Proc_GetAllLoaiTaiLieuByID", para).Tables[0];
                ep.Result = duLieu;
                return JSonHelper.ToJson(ep);
            }
            catch (Exception ex)
        ...
        public string XoaDuLieu(string id)
        {
            try
            {
                ...
                DataSet duLieu = SqlHelper.ExecuteDataset(NTSSession.GetConnectionString1(), "Delete_LoaiTaiLieu", para);
                if (duLieu.Tables[0].Rows[0][0].ToString() != "")
                {
                    duLieu.Tables[0].TableName = "LoaiTaiLieu";
