...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'TonGiao', 'TonGiaoID');

});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().TonGiaoID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().TonGiaoID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().TonGiaoID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        //{ title: "Mã", field: "MaTonGiao", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Mã", field: "MaTonGiao", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên tôn giáo", field: "TenTonGiao", formatter: 'textarea', hozAlign: "left", width: 400, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "TonGiaoID", field: "TonGiaoID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/TonGiao/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'TonGiao', 'TonGiaoID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#MaTonGiao').prop('disabled', false);
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin tôn giáo');
...
Grid1.on("rowDblClick", function (e, row) {
    ...
    SuaDuLieu(row.getData().TonGiaoID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin tôn giáo');
    const result = NTS.getAjax('/DanhMuc/TonGiao/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#TonGiaoID').value(data.TonGiaoID);
        $('#MaTonGiao').value(data.MaTonGiao);
        $('#TenTonGiao').value(data.TenTonGiao);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    saveData[0] = tempthem;
    saveData[1] = $('#TonGiaoID').value();
    saveData[2] = $('#MaTonGiao').value();
    saveData[3] = $('#TenTonGiao').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/TonGiao/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'TonGiaoID', ID: ID, TenBangHienTai: 'TonGiao', CacBangKhongXet: [] });
    //console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/TonGiao/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
