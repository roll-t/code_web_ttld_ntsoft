...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'MongMuonDN', 'MongMuonDNID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().MongMuonDNID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().MongMuonDNID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().MongMuonDNID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaMongMuonDN", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên mong muốn của doanh nghiệp đối với TTDVVL", field: "TenMongMuonDN", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "MongMuonDNID", field: "MongMuonDNID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/MongMuonDN/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'MongMuonDN', 'MongMuonDNID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới mong muốn của doanh nghiệp đối với TTDVVL');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#MongMuonDNID').val(row.getData().MongMuonDNID);
    SuaDuLieu(row.getData().MongMuonDNID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật mong muốn của doanh nghiệp đối với TTDVVL');
    const result = NTS.getAjax('/DanhMuc/MongMuonDN/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#MongMuonDNID').value(data.MongMuonDNID);
        $('#MaMongMuonDN').value(data.MaMongMuonDN);
        $('#TenMongMuonDN').value(data.TenMongMuonDN);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraKhongCoKyTuDacBiet($('#MaMongMuonDN').value())) {
        return NTS.canhbao("Mã không được chứa ký tự đăc biệt!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#MongMuonDNID').value();
    saveData[2] = $('#MaMongMuonDN').value();
    saveData[3] = $('#TenMongMuonDN').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/MongMuonDN/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#MongMuonDNID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'MongMuonDNID', ID: ID, TenBangHienTai: 'MongMuonDN', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/MongMuonDN/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
