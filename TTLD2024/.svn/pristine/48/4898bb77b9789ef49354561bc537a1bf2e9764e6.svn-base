...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'TinhTrangTimKiemVL', 'TinhTrangTimKiemVLID');

});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().TinhTrangTimKiemVLID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().TinhTrangTimKiemVLID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().TinhTrangTimKiemVLID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        //{ title: "Mã", field: "MaTinhTrangTimKiemVL", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Mã", field: "MaTinhTrangTimKiemVL", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên tình trạng tìm kiếm việc làm", field: "TenTinhTrangTimKiemVL", formatter: 'textarea', hozAlign: "left", width: 400, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "TinhTrangTimKiemVLID", field: "TinhTrangTimKiemVLID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/TinhTrangTimKiemVL/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'TinhTrangTimKiemVL', 'TinhTrangTimKiemVLID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#MaTinhTrangTimKiemVL').prop('disabled', false);
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin tình trạng tìm kiếm việc làm');
    $('#mdThemMoi').modal('show');
    $('#MaTinhTrangTimKiemVL').value('');
    $('#TenTinhTrangTimKiemVL').value('');
    $('#TinhTrangTimKiemVLID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#TinhTrangTimKiemVLID').val(row.getData().TinhTrangTimKiemVLID);
    SuaDuLieu(row.getData().TinhTrangTimKiemVLID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin tình trạng tìm kiếm việc làm');
    const result = NTS.getAjax('/DanhMuc/TinhTrangTimKiemVL/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#TinhTrangTimKiemVLID').value(data.TinhTrangTimKiemVLID);
        $('#MaTinhTrangTimKiemVL').value(data.MaTinhTrangTimKiemVL);
        $('#TenTinhTrangTimKiemVL').value(data.TenTinhTrangTimKiemVL);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    saveData[0] = tempthem;
    saveData[1] = $('#TinhTrangTimKiemVLID').value();
    saveData[2] = $('#MaTinhTrangTimKiemVL').value();
    saveData[3] = $('#TenTinhTrangTimKiemVL').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/TinhTrangTimKiemVL/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#TinhTrangTimKiemVLID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'TinhTrangTimKiemVLID', ID: ID, TenBangHienTai: 'TinhTrangTimKiemVL', CacBangKhongXet: [] });
    //console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/TinhTrangTimKiemVL/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
