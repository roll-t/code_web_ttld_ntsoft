...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'VaiTroTGLD', 'VaiTroTGLDID');
    //$('#reservationdate').inputmask('dd/mm/yyyy', { 'placeholder': 'dd/mm/yyyy' });
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().VaiTroTGLDID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().VaiTroTGLDID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().VaiTroTGLDID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaVaiTroTGLD", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên vai trò khi tham gia đình công", field: "TenVaiTroTGLD", formatter: 'textarea', hozAlign: "left", width: 400, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "VaiTroTGLDID", field: "VaiTroTGLDID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    ...
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/VaiTroTGDinhCong/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'VaiTroTGLD', 'VaiTroTGLDID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới vai trò khi tham gia đình công');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#VaiTroTGLDID').val(row.getData().VaiTroTGLDID);
    SuaDuLieu(row.getData().VaiTroTGLDID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật vai trò khi tham gia đình công');
    const result = NTS.getAjax('/DanhMuc/VaiTroTGDinhCong/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#VaiTroTGLDID').value(data.VaiTroTGLDID);
        $('#MaVaiTroTGLD').value(data.MaVaiTroTGLD);
        $('#TenVaiTroTGLD').value(data.TenVaiTroTGLD);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraKhongCoKyTuDacBiet($('#MaVaiTroTGLD').value())) {
        return NTS.canhbao("Mã không được chứa ký tự đăc biệt!")
    }
    
    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#VaiTroTGLDID').value();
    saveData[2] = $('#MaVaiTroTGLD').value();
    saveData[3] = $('#TenVaiTroTGLD').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/VaiTroTGDinhCong/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#VaiTroTGLDID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'VaiTroTGLDID', ID: ID, TenBangHienTai: 'VaiTroTGLD', CacBangKhongXet: [] });
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/VaiTroTGDinhCong/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
