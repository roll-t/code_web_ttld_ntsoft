...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'ViTheViecLam', 'ViTheViecLamID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().ViTheViecLamID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().ViTheViecLamID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().ViTheViecLamID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaViTheViecLam", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên vị thế việc làm", field: "TenViTheViecLam", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "ViTheViecLamID", field: "ViTheViecLamID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/ViTheViecLam/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'ViTheViecLam', 'ViTheViecLamID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới vị thế việc làm');
    $('#mdThemMoi').modal('show');
    $('#MaViTheViecLam').value('');
    $('#TenViTheViecLam').value('');
    $('#ViTheViecLamID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#ViTheViecLamID').val(row.getData().ViTheViecLamID);
    SuaDuLieu(row.getData().ViTheViecLamID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật vị thế việc làm');
    const result = NTS.getAjax('/DanhMuc/ViTheViecLam/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#ViTheViecLamID').value(data.ViTheViecLamID);
        $('#MaViTheViecLam').value(data.MaViTheViecLam);
        $('#TenViTheViecLam').value(data.TenViTheViecLam);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraKhongCoKyTuDacBiet($('#MaViTheViecLam').value())) {
        return NTS.canhbao("Mã chỉ không được chứa ký tự đăc biệt!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#ViTheViecLamID').value();
    saveData[2] = $('#MaViTheViecLam').value();
    saveData[3] = $('#TenViTheViecLam').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/ViTheViecLam/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#ViTheViecLamID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'ViTheViecLamID', ID: ID, TenBangHienTai: 'ViTheViecLam', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/ViTheViecLam/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
