...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'LyDoMuonHuongTCTN', 'LyDoMuonHuongTCTNID');

});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().LyDoMuonHuongTCTNID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().LyDoMuonHuongTCTNID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().LyDoMuonHuongTCTNID);
}
var fmMoTaNgan = function (cell) {
    var ID = cell.getData().LyDoMuonHuongTCTNID;
...
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        //{ title: "Mã", field: "MaLyDoMuonHuongTCTN", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Mã", field: "MaLyDoMuonHuongTCTN", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên lý do muốn hưởng trợ cấp thất nghiệp", field: "TenLyDoMuonHuongTCTN", formatter: fmMoTaNgan, hozAlign: "left", width: 400, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "LyDoMuonHuongTCTNID", field: "LyDoMuonHuongTCTNID", width: 0, visible: false }
    ],
    locale: true,
...
function XemChiTietGhiChu(ID) {
    ...
    const result = NTS.getAjax("/DanhMuc/LyDoMuonHuongTCTN/LoadDuLieuSua", { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#NoiDungGhiChu_CT').html(data.TenLyDoMuonHuongTCTN);
    } else {
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/LyDoMuonHuongTCTN/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'LyDoMuonHuongTCTN', 'LyDoMuonHuongTCTNID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#MaLyDoMuonHuongTCTN').prop('disabled', false);
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin lý do muốn hưởng trợ cấp thất nghiệp');
    $('#mdThemMoi').modal('show');
    $('#MaLyDoMuonHuongTCTN').value('');
    $('#TenLyDoMuonHuongTCTN').value('');
    $('#LyDoMuonHuongTCTNID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#LyDoMuonHuongTCTNID').val(row.getData().LyDoMuonHuongTCTNID);
    SuaDuLieu(row.getData().LyDoMuonHuongTCTNID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin lý do muốn hưởng trợ cấp thất nghiệp');
    const result = NTS.getAjax('/DanhMuc/LyDoMuonHuongTCTN/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#LyDoMuonHuongTCTNID').value(data.LyDoMuonHuongTCTNID);
        $('#MaLyDoMuonHuongTCTN').value(data.MaLyDoMuonHuongTCTN);
        $('#TenLyDoMuonHuongTCTN').value(data.TenLyDoMuonHuongTCTN);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    saveData[0] = tempthem;
    saveData[1] = $('#LyDoMuonHuongTCTNID').value();
    saveData[2] = $('#MaLyDoMuonHuongTCTN').value();
    saveData[3] = $('#TenLyDoMuonHuongTCTN').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/LyDoMuonHuongTCTN/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#LyDoMuonHuongTCTNID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'LyDoMuonHuongTCTNID', ID: ID, TenBangHienTai: 'LyDoMuonHuongTCTN', CacBangKhongXet: [] });
    //console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/LyDoMuonHuongTCTN/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
