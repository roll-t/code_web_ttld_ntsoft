...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'DTChinhSach', 'DTChinhSachID');

});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().DTChinhSachID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().DTChinhSachID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().DTChinhSachID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        //{ title: "Mã", field: "MaDTChinhSach", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Mã", field: "MaDTChinhSach", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên đối tượng chính sách", field: "TenDTChinhSach", formatter: 'textarea', hozAlign: "left", width: 400, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "DTChinhSachID", field: "DTChinhSachID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/DTChinhSach/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'DTChinhSach', 'DTChinhSachID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#MaDTChinhSach').prop('disabled', false);
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin đối tượng chính sách');
    $('#mdThemMoi').modal('show');
    $('#MaDTChinhSach').value('');
    $('#TenDTChinhSach').value('');
    $('#DTChinhSachID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#DTChinhSachID').val(row.getData().DTChinhSachID);
    SuaDuLieu(row.getData().DTChinhSachID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin đối tượng chính sách');
    const result = NTS.getAjax('/DanhMuc/DTChinhSach/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#DTChinhSachID').value(data.DTChinhSachID);
        $('#MaDTChinhSach').value(data.MaDTChinhSach);
        $('#TenDTChinhSach').value(data.TenDTChinhSach);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    saveData[0] = tempthem;
    saveData[1] = $('#DTChinhSachID').value();
    saveData[2] = $('#MaDTChinhSach').value();
    saveData[3] = $('#TenDTChinhSach').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/DTChinhSach/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#DTChinhSachID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'DTChinhSachID', ID: ID, TenBangHienTai: 'DTChinhSach', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/DTChinhSach/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
