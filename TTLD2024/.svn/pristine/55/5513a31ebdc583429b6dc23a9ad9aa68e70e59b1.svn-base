...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'QuanHe', 'QuanHeID');

});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().QuanHeID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().QuanHeID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().QuanHeID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        //{ title: "Mã", field: "MaQuanHe", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Mã", field: "MaQuanHe", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên quan hệ", field: "TenQuanHe", formatter: 'textarea', hozAlign: "left", width: 400, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "QuanHeID", field: "QuanHeID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/QuanHeChuHo/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'QuanHe', 'QuanHeID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#MaQuanHe').prop('disabled', false);
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin quan hệ');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#QuanHeID').val(row.getData().QuanHeID);
    SuaDuLieu(row.getData().QuanHeID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin quan hệ');
    const result = NTS.getAjax('/DanhMuc/QuanHeChuHo/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#QuanHeID').value(data.QuanHeID);
        $('#MaQuanHe').value(data.MaQuanHe);
        $('#TenQuanHe').value(data.TenQuanHe);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    saveData[0] = tempthem;
    saveData[1] = $('#QuanHeID').value();
    saveData[2] = $('#MaQuanHe').value();
    saveData[3] = $('#TenQuanHe').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/QuanHeChuHo/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'QuanHeID', ID: ID, TenBangHienTai: 'QuanHe', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/QuanHeChuHo/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
