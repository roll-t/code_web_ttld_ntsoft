...
$(function () {
    //LoadCombo();
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'Slide', 'SlideID');
});
function LoadCombo() {
    setTimeout(function () {        
...
var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().SlideID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().SlideID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().SlideID);
}
...
var fmMoTaNgan_TieuDe = function (cell) {
    var ID = cell.getData().SlideID;
    var ghiChu = cell.getValue();
    if (ghiChu != "") {
        if (ghiChu.length > 40) {
            ...
            return `
                        <div class="col-md-12" style="padding-right: 0px; padding-left: 0px; padding-bottom:4px;">
                            <div class="Content-text">${ghiChu}<span class='btnXemMoTaNgan_TieuDe' style='color:var(--tblr-primary);'  title="Xem chi tiết ghi chú" data='${ID}' data-loai='TC'>Xem thêm</span></div>
                        </div>`;
        } else {
            return `
                    <div class="col-md-12">
...
var fmMoTaNgan_DuongDan = function (cell) {
    var ID = cell.getData().SlideID;
    var ghiChu = cell.getValue();
    if (ghiChu != "") {
        if (ghiChu.length > 40) {
            ...
            return `
                        <div class="col-md-12" style="padding-right: 0px; padding-left: 0px; padding-bottom:4px;">
                            <div class="Content-text">${ghiChu}<span class='btnXemMoTaNgan_DuongDan' style='color:var(--tblr-primary);'  title="Xem chi tiết ghi chú" data='${ID}' data-loai='TC'>Xem thêm</span></div>
                        </div>`;
        } else {
            return `
                    <div class="col-md-12">
...
var Grid1 = new Tabulator("#GridSlide", {
    layout: "fitColumns",
    pagination: true,
    paginationSize: 50,
    ...
    columns: [
        ...
        { title: "SlideID", field: "SlideID", formatter: "textarea", width: 100, visible: false },
        { title: "Số thứ tự", field: "HienUuTien", hozAlign: "left", formatter: "textarea", width: 100, vertAlign: "middle", headerHozAlign: "center" },
        { title: "Tiêu đề", field: "TieuDe", hozAlign: "left", formatter: fmMoTaNgan_TieuDe, minWidth: 300, vertAlign: "middle", headerHozAlign: "center" },
        { title: "Mô tả", field: "MoTa", hozAlign: "left", formatter: fmMoTaNgan, minWidth: 380, vertAlign: "middle", headerHozAlign: "center" },
        { title: "Đường dẫn", field: "DuongDan", hozAlign: "left", formatter: fmMoTaNgan_DuongDan, minWidth: 380, vertAlign: "middle", headerHozAlign: "center" },
...
$(document).on('click', '.btnXemMoTaNgan', function () {
    XemChiTietGhiChu($(this).attr('data'), "MoTa");
});

// Xem chi tiết nội dung 
$(document).on('click', '.btnXemMoTaNgan_TieuDe', function () {
    XemChiTietGhiChu($(this).attr('data'), "TieuDe");
});
// Xem chi tiết nội dung 
$(document).on('click', '.btnXemMoTaNgan_DuongDan', function () {
    XemChiTietGhiChu($(this).attr('data'), "DuongDan");
});
function XemChiTietGhiChu(ID, Loai) {
    debugger
    $("#mdXemThem").modal('show');
    const result = NTS.getAjax("/CongThongTin/Slide/LoadDuLieuSua", { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        if (Loai == "MoTa") {
            $('#tieuDeModalCT').text('Chi tiết mô tả');
            $('#NoiDungGhiChu_CT').html(data.MoTa);
        }
        if (Loai == "TieuDe") {
            $('#tieuDeModalCT').text('Chi tiết tiêu đề');

            $('#NoiDungGhiChu_CT').html(data.TieuDe);
        }
        if (Loai == "DuongDan") {
            $('#tieuDeModalCT').text('Chi tiết đường dẫn');

            $('#NoiDungGhiChu_CT').html(data.DuongDan);
        }
    } else {
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/CongThongTin/Slide/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'Slide', 'SlideID');
...
$(document).on('click', '#btnThemMoi', function () {
    ...
    $('#lblTieuDeThemMoi').text('Thêm mới Slide');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#SlideID').val(row.getData().SlideID);
    SuaDuLieu(row.getData().SlideID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật slide');
    const result = NTS.getAjax('/CongThongTin/Slide/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        resetUploadFile_TaiLieu();
        ...
        $('#HienUuTien').value(data.HienUuTien);
        $('#TrangThai').value(data.TrangThai);

        UpdateLabelDangSD('#TrangThai');
        $('#txtDuongDanFileVB_tailieu').value(data.IMG);
        $('#mdThemMoi').modal('show');
        tempthem = "sua";
        ChuoiPath = "";
        ChuoiPath = data.IMG;
        ...
        $('#tieuDeModal').text('Cập nhật slide');
    }
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if ($('#txtDuongDanFileVB_tailieu').value() == '') {
        NTS.canhbao('Hình ảnh slide không được bỏ trống!');
        return false;
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $("#SlideID").value();
    saveData[2] = $("#TieuDe").value();
    saveData[3] = $("#MoTa").value();
    saveData[4] = $("#DuongDan").value();
    saveData[5] = $("#HienUuTien").value();
    ...
    saveData[7] = duongDan;
    var result = NTS.getAjax('/CongThongTin/Slide/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
function XoaDuLieu(ID) {
    ...
    CanhBaoXoa(() => {
        const result = NTS.getAjax('/CongThongTin/Slide/XoaDuLieu', { id: ID });
        if (!result.Err) {
            LoadDataTable();
            NTS.thanhcong(result.Msg);
        }
...
$(document).on('change', '#fileVB_tailieu', function () {
    UploadTaiLieu_us('IMG'); //hàm dùng chung ở us TaiLieu
});
function UploadTaiLieu_us(pathChiTiet) {
    ...
    if (data.length > 0) {
        ...
        if (tempthemVB != "them") {
            let result = NTS.getAjax('/CongThongTin/DungChung/LuuVanBan_DinhKem', { PathTemp: data, ID: $('#hdfTaiLieuLuuGiuID_us').value(), PathChiTiet: pathChiTiet, bangDk: 'Slide', cotDk: 'SlideID', cotDinhKem: 'IMG' });
            if (!result.Err) {
...
/*$(document).on('click', '#btnXoaHetTepVB', function () {
    ...
    var kq = NTS.getAjax('json', '/View/CongThongTin/Slide.aspx/XoaFile', { PathFile: $('#txtDuongDanFileVB_tailieu').value() });
    $('#txtDuongDanFileVB_tailieu').value('');
    $('#TenFile').text('');
    AnHienXoaHetTepVB();
});*/
/*function XoaAnhSlide() {
    if (!ntspermiss.xoa) {
        NTS.canhbao("User bạn đang sử dụng không thể thực hiện thao tác xóa. Vui lòng kiểm tra lại.");
        return false;
    }
...
$(document).on('click', '.delete-file-attachments', function () {
    ...
    CanhBaoXoaDinhKem(() => {
        let result = NTS.getAjax('/CongThongTin/DungChung/XoaDinhKem', { ID: id, duongDan: duongDan.attr('data-url-file'), bangDk: bang, cotDk: cot, loai: '', tenCotTrongDB: 'IMG' });
        if (!result.Err) {
