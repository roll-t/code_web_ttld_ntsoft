...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'ChucDanh', 'ChucDanhID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().ChucDanhID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().ChucDanhID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().ChucDanhID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã chức danh", field: "MaChucDanh", formatter: 'textarea', width: 150, vertAlign: "middle", minWidth: 80, headerHozAlign: "center", hozAlign: "center" },
        { title: "Tên chức danh", field: "TenChucDanh", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "ChucDanhID", field: "ChucDanhID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/ChucDanh/GetAll", { id : '1' });
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'ChucDanh', 'ChucDanhID');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#ChucDanhID').val(row.getData().ChucDanhID);
    SuaDuLieu(row.getData().ChucDanhID);
});
...
function SuaDuLieu(ID) {
    ...
    const result = NTS.getAjax('/DanhMuc/ChucDanh/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#ChucDanhID').value(data.ChucDanhID);
        $('#MaChucDanh').value(data.MaChucDanh);
        $('#TenChucDanh').value(data.TenChucDanh);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraNhapSo($('#MaChucDanh').value())) {
        return NTS.canhbao("Mã chỉ được chứa ký tự số!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#ChucDanhID').value();
    saveData[2] = $('#MaChucDanh').value();
    saveData[3] = $('#TenChucDanh').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/ChucDanh/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'ChucDanhID', ID: ID, TenBangHienTai: 'ChucDanh', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/ChucDanh/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
