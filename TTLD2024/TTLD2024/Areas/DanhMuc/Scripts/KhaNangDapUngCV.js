...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'KhaNangDapUngCV', 'KhaNangDapUngCVID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().KhaNangDapUngCVID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().KhaNangDapUngCVID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().KhaNangDapUngCVID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaKhaNangDapUngCV", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên khả năng đáp ứng công việc", field: "TenKhaNangDapUngCV", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "KhaNangDapUngCVID", field: "KhaNangDapUngCVID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/KhaNangDapUngCV/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'KhaNangDapUngCV', 'KhaNangDapUngCVID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới khả năng đáp ứng công việc');
    $('#mdThemMoi').modal('show');
    $('#MaKhaNangDapUngCV').value('');
    $('#TenKhaNangDapUngCV').value('');
    $('#KhaNangDapUngCVID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#KhaNangDapUngCVID').val(row.getData().KhaNangDapUngCVID);
    SuaDuLieu(row.getData().KhaNangDapUngCVID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật khả năng đáp ứng công việc');
    const result = NTS.getAjax('/DanhMuc/KhaNangDapUngCV/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#KhaNangDapUngCVID').value(data.KhaNangDapUngCVID);
        $('#MaKhaNangDapUngCV').value(data.MaKhaNangDapUngCV);
        $('#TenKhaNangDapUngCV').value(data.TenKhaNangDapUngCV);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraKhongCoKyTuDacBiet($('#MaKhaNangDapUngCV').value())) {
        return NTS.canhbao("Mã chỉ không được chứa ký tự đăc biệt!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#KhaNangDapUngCVID').value();
    saveData[2] = $('#MaKhaNangDapUngCV').value();
    saveData[3] = $('#TenKhaNangDapUngCV').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/KhaNangDapUngCV/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#KhaNangDapUngCVID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'KhaNangDapUngCVID', ID: ID, TenBangHienTai: 'KhaNangDapUngCV', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/KhaNangDapUngCV/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
