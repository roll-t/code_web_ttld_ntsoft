...
var btnThaoTac = function (cell, formatterParams, onRendered) {
    return `<button type='button' class='btn btn-xs btn-success btnXemHSLuuTru' title="Xem" data='${cell.getRow().getData().ViecTimNguoiID}'><i class="fa fa-eye"></i></button>&ensp;
            <button type="button" class='btn btn-xs btn-danger btnBoLuuHS' title="Bỏ lưu" data='${cell.getRow().getData().ViecLamDaLuuID}' ><i class="fas fa-trash-alt"></i></button>
            `;
};
...
var table = new Tabulator("#Grid1", {
    ...
    columns: [ //Define Table Columns
        { title: "Thao tác", field: 'ThaoTac', hozAlign: "center", formatter: btnThaoTac, width: 80, headerSort: false, vertAlign: "middle", headerHozAlign: "center", vertAlign: "middle" },
        ...
        { title: "Ngày thao tác", field: "NgayThaoTac", hozAlign: "center", visible: true, width: 150, headerHozAlign: "center", formatter: "textarea", vertAlign: "middle" },
    ],
...
function LoadGrid1() {
    var GetAll = NTS.getAjax("/UngVien/ViecLamDangLuuTru/GetAll", {}).Result;
    console.log(GetAll);
    if (GetAll.length > 0) {
        table.clearData();
...
function XemTinTuyenDung(ID) {
    ...
    var result = NTS.getAjax('/UngVien/ViecLamDangLuuTru/GetThongTinViecLamByID', { ID: ID }).Result;
    let data = result[0];
...
function XoaViecLamLuuTru(ID) {
    CanhBaoXoa(() => {
        var result = NTS.getAjax('/UngVien/ViecLamDangLuuTru/XoaViecLamDangLuuTru', { ID: ID });
        if (!result.Err) {
            LoadGrid1();
