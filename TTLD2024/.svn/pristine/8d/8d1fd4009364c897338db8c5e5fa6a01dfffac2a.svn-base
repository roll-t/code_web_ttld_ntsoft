...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'NhomVTVL', 'NhomVTVLID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().NhomVTVLID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().NhomVTVLID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().NhomVTVLID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaNhomVTVL", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên nhóm vị trí việc làm", field: "TenNhomVTVL", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "NhomVTVLID", field: "NhomVTVLID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/NhomVTVL/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'NhomVTVL', 'NhomVTVLID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới nhóm vị trí việc làm');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#NhomVTVLID').val(row.getData().NhomVTVLID);
    SuaDuLieu(row.getData().NhomVTVLID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông nhóm vị trí việc làm');
    const result = NTS.getAjax('/DanhMuc/NhomVTVL/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#NhomVTVLID').value(data.NhomVTVLID);
        $('#MaNhomVTVL').value(data.MaNhomVTVL);
        $('#TenNhomVTVL').value(data.TenNhomVTVL);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraKhongCoKyTuDacBiet($('#MaNhomVTVL').value())) {
        return NTS.canhbao("Mã không được chứa ký tự đăc biệt!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#NhomVTVLID').value();
    saveData[2] = $('#MaNhomVTVL').value();
    saveData[3] = $('#TenNhomVTVL').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/NhomVTVL/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'NhomVTVLID', ID: ID, TenBangHienTai: 'NhomVTVL', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/NhomVTVL/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
