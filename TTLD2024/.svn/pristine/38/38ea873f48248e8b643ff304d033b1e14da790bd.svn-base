...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'ChuDeCV', 'ChuDeID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().ChuDeID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().ChuDeID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().ChuDeID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "ChuDeCode", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên chức vụ", field: "TenChuDe", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "ChuDeID", field: "ChuDeID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/CongThongTin/ChuDeCV/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'ChuDeCV', 'ChuDeID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin chức vụ');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#ChuDeID').val(row.getData().ChuDeID);
    SuaDuLieu(row.getData().ChuDeID);
});
...
function SuaDuLieu(ID) {
    ...
    const result = NTS.getAjax('/CongThongTin/ChuDeCV/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#ChuDeID').value(data.ChuDeID);
        $('#ChuDeCode').value(data.ChuDeCode);
        $('#TenChuDe').value(data.TenChuDe);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraNhapSo($('#ChuDeCode').value())) {
        return NTS.canhbao("Mã chỉ được chứa ký tự số!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#ChuDeID').value();
    saveData[2] = $('#ChuDeCode').value();
    saveData[3] = $('#TenChuDe').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/CongThongTin/ChuDeCV/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'ChuDeID', ID: ID, TenBangHienTai: 'ChuDeCV', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/CongThongTin/ChuDeCV/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
