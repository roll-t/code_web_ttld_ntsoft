...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'HinhThucNLDLVNN', 'HinhThucNLDLVNNID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().HinhThucNLDLVNNID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().HinhThucNLDLVNNID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().HinhThucNLDLVNNID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã hình thức người lao động làm việc ở nước ngoài", field: "MaHinhThucNLDLVNN", formatter: 'textarea', width: 250, vertAlign: "middle", minWidth: 80, headerHozAlign: "center", hozAlign: "center" },
        { title: "Tên hình thức người lao động làm việc ở nước ngoài", field: "TenHinhThucNLDLVNN", formatter: 'textarea', hozAlign: "left", width: 272, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "HinhThucNLDLVNNID", field: "HinhThucNLDLVNNID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/HinhThucNLDLVNN/GetAll", { id : '1' });
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'HinhThucNLDLVNN', 'HinhThucNLDLVNNID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin chức danh');
    $('#mdThemMoi').modal('show');
    $('#MaHinhThucNLDLVNN').value('');
    $('#TenHinhThucNLDLVNN').value('');
    $('#HinhThucNLDLVNNID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#HinhThucNLDLVNNID').val(row.getData().HinhThucNLDLVNNID);
    SuaDuLieu(row.getData().HinhThucNLDLVNNID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin chức vụ');
    const result = NTS.getAjax('/DanhMuc/HinhThucNLDLVNN/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#HinhThucNLDLVNNID').value(data.HinhThucNLDLVNNID);
        $('#MaHinhThucNLDLVNN').value(data.MaHinhThucNLDLVNN);
        $('#TenHinhThucNLDLVNN').value(data.TenHinhThucNLDLVNN);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraNhapSo($('#MaHinhThucNLDLVNN').value())) {
        return NTS.canhbao("Mã chỉ được chứa ký tự số!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#HinhThucNLDLVNNID').value();
    saveData[2] = $('#MaHinhThucNLDLVNN').value();
    saveData[3] = $('#TenHinhThucNLDLVNN').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/HinhThucNLDLVNN/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#HinhThucNLDLVNNID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'HinhThucNLDLVNNID', ID: ID, TenBangHienTai: 'HinhThucNLDLVNN', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/HinhThucNLDLVNN/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
