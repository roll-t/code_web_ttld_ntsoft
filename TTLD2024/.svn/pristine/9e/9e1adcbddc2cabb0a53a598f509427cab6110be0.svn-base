...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'TrinhDoTinHoc', 'TrinhDoTinHocID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().TrinhDoTinHocID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().TrinhDoTinHocID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().TrinhDoTinHocID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaTrinhDoTinHoc", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên trình độ tin học", field: "TenTrinhDoTinHoc", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "TrinhDoTinHocID", field: "TrinhDoTinHocID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/TrinhDoTinHoc/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'TrinhDoTinHoc', 'TrinhDoTinHocID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới trình độ tin học');
    $('#mdThemMoi').modal('show');
    $('#MaTrinhDoTinHoc').value('');
    $('#TenTrinhDoTinHoc').value('');
    $('#TrinhDoTinHocID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#TrinhDoTinHocID').val(row.getData().TrinhDoTinHocID);
    SuaDuLieu(row.getData().TrinhDoTinHocID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật trình độ tin học');
    const result = NTS.getAjax('/DanhMuc/TrinhDoTinHoc/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#TrinhDoTinHocID').value(data.TrinhDoTinHocID);
        $('#MaTrinhDoTinHoc').value(data.MaTrinhDoTinHoc);
        $('#TenTrinhDoTinHoc').value(data.TenTrinhDoTinHoc);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    saveData[0] = tempthem;
    saveData[1] = $('#TrinhDoTinHocID').value();
    saveData[2] = $('#MaTrinhDoTinHoc').value();
    saveData[3] = $('#TenTrinhDoTinHoc').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/TrinhDoTinHoc/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#TrinhDoTinHocID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'TrinhDoTinHocID', ID: ID, TenBangHienTai: 'TrinhDoTinHoc', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/TrinhDoTinHoc/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
