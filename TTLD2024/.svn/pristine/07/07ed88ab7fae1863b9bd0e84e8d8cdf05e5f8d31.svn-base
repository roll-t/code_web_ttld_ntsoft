...
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Trình độ PT cao nhất", field: "TenTrinhDoHocVan", formatter: 'textarea', vertAlign: "middle", minWidth: 200, headerHozAlign: "center" },
...
async function LoadDataTable() {
    ...
    Grid1.clearData();
    const GetAll = await NTS.getAjaxAsync("/QuanLy/ThuThapCungLaoDongBanDau/GetAll", { data: saveData });
    if (!GetAll.Err) {
...
async function LoadThongTinDonVi() {
    try {
...
function XoaDuLieuCungLaoDong(ID) {
    ...
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/QuanLy/ThuThapCungLaoDongBanDau/XoaDuLieuCungLaoDong', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
...
async function XuatMau01_TT01(ID) {
    var data = await NTS.getAjaxAsync('/QuanLy/ThuThapCungLaoDongBanDau/XuatMau01', { id: ID });
    //window.open(data);
    try {
...
async function XuatMau03_TT11(ID) {
    var data = await NTS.getAjaxAsync('/QuanLy/ThuThapCungLaoDongBanDau/XuatMau03', { id: ID });
    //window.open(data);
    try {
        $('#divNoiDung_us').attr("src", data.replace(".docx", ".pdf"));
        $('#modal_xemtruockhiin_us').modal('show');
...
$('#btnExport').on('click', async function () {
    ...
    saveData[6] = $('#DenNgay_TimKiem_US').value();
    saveData[7] = $('#HoGiaDinhID_TimKiem_us').value();
    var kq = await NTS.getAjaxAsync('/QuanLy/ThuThapCungLaoDongBanDau/XuatExcel_DSThuThapCungLaoDongBanDau', { data: saveData });
    if (!kq.Err) {
        window.open(kq);
    } else {
        NTS.loi(kq.Msg);
