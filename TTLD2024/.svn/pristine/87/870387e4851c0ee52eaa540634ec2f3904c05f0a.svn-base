...
$(function () {
    LoadDataTable();
    checkMacDinhSD('.checkMacDinhSD', 'TinhTrangTaiLieu', 'TinhTrangTaiLieuID');
});

var fmThaoTac = function (cell) {
    return formaterbtnThaoTac(cell.getData().TinhTrangTaiLieuID);
}
var fmDangSD = function (cell) {
    return formaterDangSD(cell.getValue(), cell.getData().TinhTrangTaiLieuID);
}
var fmMacDinhSD = function (cell) {
    return formaterMacDinhSD(cell.getValue(), cell.getData().TinhTrangTaiLieuID);
}
var Grid1 = new Tabulator("#Grid1", {
    ...
    columns: [
        ...
        { title: "Mã", field: "MaTinhTrangTaiLieu", formatter: 'textarea', width: 80, vertAlign: "middle", minWidth: 80, headerHozAlign: "center" },
        { title: "Tên tình trạng tài liệu", field: "TenTinhTrangTaiLieu", formatter: 'textarea', hozAlign: "left", width: 472, vertAlign: "middle", headerHozAlign: "center" },
        ...
        { title: "TinhTrangTaiLieuID", field: "TinhTrangTaiLieuID", width: 0, visible: false }
    ],
    locale: true,
...
function LoadDataTable() {
    Grid1.clearData();
    const GetAll = NTS.getAjax("/DanhMuc/TinhTrangTaiLieu/GetAll", {});
    if (!GetAll.Err) {
...
checkDangSD('.checkDangSD', 'TinhTrangTaiLieu', 'TinhTrangTaiLieuID');
...
$(document).on('click', '#btnThemMoi', function () {
    $('#lblTieuDeThemMoi').text('Thêm mới thông tin tình trạng tài liệu');
    $('#mdThemMoi').modal('show');
    $('#MaTinhTrangTaiLieu').value('');
    $('#TenTinhTrangTaiLieu').value('');
    $('#TinhTrangTaiLieuID').value('');
    $('#DienGiai').value('');
...
Grid1.on("rowDblClick", function (e, row) {
    $('#TinhTrangTaiLieuID').val(row.getData().TinhTrangTaiLieuID);
    SuaDuLieu(row.getData().TinhTrangTaiLieuID);
});
...
function SuaDuLieu(ID) {
    ...
    $('#lblTieuDeThemMoi').text('Cập nhật thông tin tình trạng tài liệu');
    const result = NTS.getAjax('/DanhMuc/TinhTrangTaiLieu/LoadDuLieuSua', { id: ID });
    if (!result.Err && result.Result != null) {
        let data = result.Result[0];
        $('#TinhTrangTaiLieuID').value(data.TinhTrangTaiLieuID);
        $('#MaTinhTrangTaiLieu').value(data.MaTinhTrangTaiLieu);
        $('#TenTinhTrangTaiLieu').value(data.TenTinhTrangTaiLieu);
...
$(document).on('click', '#btnLuuVaDong', function () {
    ...
    if (KiemTraNhapSo($('#MaTinhTrangTaiLieu').value())) {
        return NTS.canhbao("Mã chỉ được chứa ký tự số!");
    }

    var saveData = new Array();
    saveData[0] = tempthem;
    saveData[1] = $('#TinhTrangTaiLieuID').value();
    saveData[2] = $('#MaTinhTrangTaiLieu').value();
    saveData[3] = $('#TenTinhTrangTaiLieu').value();
    saveData[4] = $('#DienGiai').value();
    saveData[5] = $('#TrangThai').value();
    var result = NTS.getAjax('/DanhMuc/TinhTrangTaiLieu/LuuThongTin', { data: saveData });
    if (!result.Err) {
        LoadDataTable();
...
$(document).on('click', '.btnSuaGrid1', function () {
    $('#TinhTrangTaiLieuID').val($(this).attr('data'));
...
function XoaDuLieu(ID) {
    ...
    const result_ktxoa = NTS.getAjax('/DanhMuc/DungChung/KiemTraXoaDT', { TenCot: 'TinhTrangTaiLieuID', ID: ID, TenBangHienTai: 'TinhTrangTaiLieu', CacBangKhongXet: [] });
    console.log(result_ktxoa)
    if (!result_ktxoa.Err) {
        if (result_ktxoa.Result == null || result_ktxoa.Result == "") {
            CanhBaoXoa(() => {
                const result = NTS.getAjax('/DanhMuc/TinhTrangTaiLieu/XoaDuLieu', { id: ID });
                if (!result.Err) {
                    LoadDataTable();
                    NTS.thanhcong(result.Msg);
                }
